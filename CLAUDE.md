...
- **Relaxation ↔ sandbox-active 커플링 (hard gate)**: foreground plugin read-relaxation (`src/tools/executor.ts` executeOne) 은 `hostClassifiesRisk` ON **AND `sandboxActiveProvider()` (= `isAsrtSandboxActive`) true** 일 때만 발화. relaxation 은 pre-exec ask 를 제거하고 effect-boundary 에 의존하는데, effect-boundary 는 OS 샌드박스가 **ACTIVE** 일 때만 off-hostApi mutation 을 contain 하므로 — sandbox 비활성(degraded/off) 호스트에서 relaxing 은 pre-exec ask 보다 약하다. 따라서 sandbox 비활성 → relaxation 미발화 → 기존 pre-exec ask(Phase-0 modal) 유지(known-safe). 결과: macOS(sandbox active) → relax(clean UX 불변); degraded/sandbox-off 호스트 → pre-exec ask. 이 커플링이 `hostClassifiesRisk`-ON 을 전 플랫폼에서 안전하게 만든다. 두 production 생성 지점(`boot.ts` pluginSurfaceExecutor + `engine/conversation-loop.ts`)이 `isAsrtSandboxActive` 를 주입.
