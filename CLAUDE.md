...
- **Plugin egress SOT**: `local-indexer` 의 long-lived Python worker provider egress 는 broker + `hostApi.hostFetch` chokepoint 로 수렴한다. `ep-api` 는 Python worker 가 없고 Playwright 브라우저 SSO + 사내망 HTTP/HTTPS endpoint 를 직접 다루는 예외이므로, 허용 호스트는 `lvis-plugin-ep-api/plugin.json` 의 `networkAccess.allowedDomains` 가 SOT 이다. 새 direct egress 는 manifest allow-list 없이 추가하지 않는다.
