...
2. **Two naming namespaces** — Plugin IDs use dot format (`com.ep.meeting-recorder`); LLM tool names use underscore-only (`meeting_start`). No runtime conversion — methods must be declared in underscore form in the manifest.
...
Q12 permission policy 구현은 `docs/architecture/q12-permission-policy-design.md` 와 `docs/architecture/architecture.md` §6.3 을 single source of truth 로 따른다.
...
- Category 미선언 plugin grace 는 boot-warn only 이며 **hard removal date 는 2026-05-23 KST**. 이후 미선언 plugin 은 load/registration hard fail 로 전환한다. 연장은 CLAUDE.md 와 Q12 design 문서에 명시된 공식 결정 없이는 금지한다.
