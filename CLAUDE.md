...
- Plugin manifest `toolSchemas[*].category` 는 선택적 metadata 이며, 선언 시 `read | write | shell | network` 만 허용한다. `meta` 는 host builtin category 전용이며 plugin manifest 에서 금지한다. Effective category 는 host-side `inspectHostRisk`/executor policy 가 invocation 별로 산출한다.
