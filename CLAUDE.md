...
- **Windows (srt-win)**: 동일 gate 에 합류 (`LVIS_SANDBOX_WINDOWS` opt-in 제거). srt-win.exe 는 **번들** (asarUnpack vendor/**, **다운로드 없음** — 활성화는 오직 install(UAC)+relogin 만 트리거) 이나 1회 UAC 설치 + 재로그인 필요. **win32-not-ready 시 hard-throw 안 함** (first-run brick 방지) — `isAsrtSandboxActive()` FALSE 유지(호스트 셸 도구 비격리) + "NO OS isolation until setup completes" loud 신호. ready 면 **network-only capability** (`confines.filesystem === false`) 발행 → reviewer `sandboxRelaxesCategory` live (network relax, write/shell relax 안 함).
- **Windows consent UX**: Settings → 권한 토글 ON → `sandboxWindowsStatus` IPC (read-only, UAC 없음) 로 readiness 조회 → `ready === false` 면 명시적 consent 패널 (경고 + verbatim install instructions + "Install now" 버튼). **auto-UAC 금지** — 버튼만이 `sandboxWindowsInstall` IPC (MUTATING, sender-frame-guarded) 를 호출하고 이것이 ASRT `installWindowsSandbox` 의 단일 self-elevating UAC 를 트리거하는 유일한 privilege-escalation 진입점. UAC 거부 → `{cancelled:true}` → 토글 revert + 배너. 성공 → relogin 지시 + pending 비주얼.
- **Asymmetric reviewer relaxation**: win32 network-only `confines` 가 reviewer 를 비대칭으로 relax — `network` 만 relax, `write`/`shell`/`read` 는 relax 안 함 (Windows FS jail 없음). ToolApprovalDialog 라벨도 confines-aware ("네트워크만 격리 [net:✓ fs:✗ proc:✗]", display-only).
...
- **Relaxation ↔ sandbox FILESYSTEM-CONTAINMENT 커플링 (hard gate, confines-aware)**: foreground plugin read-relaxation (`src/tools/executor.ts` executeOne) 은 `hostClassifiesRisk` ON **AND `sandboxFsContainedProvider()` (= `isActiveSandboxFilesystemContained`) true** 일 때만 발화. relaxation 은 pre-exec ask (off-hostApi `node:fs` WRITE residual 을 gate 하는 것 포함) 를 제거하고 effect-boundary 에 의존하는데, effect-boundary 는 활성 샌드박스가 **filesystem 을 contain** (`confines.filesystem === true`) 할 때만 그 residual 을 contain 한다. 따라서 단순 "sandbox active" 가 아니라 **filesystem-contained** 여야 한다 — Windows srt-win 은 **NETWORK-ONLY** (`confines.filesystem === false`) 라 active 여도 egress 만 contain 하고 FS-write residual 은 contain 못 함. filesystem-contained 아닌 호스트(degraded/off/network-only)에서 relaxing 은 pre-exec ask 보다 약하므로 → relaxation 미발화 → 기존 pre-exec approval ask 유지(known-safe). 결과: macOS/Linux(filesystem-contained ASRT) → relax(clean UX 불변); degraded/sandbox-off/**Windows network-only** 호스트 → pre-exec ask. 따라서 "relaxation 발효 ⇒ off-hostApi FS-write residual contained" 불변식이 **전 플랫폼에서 TRUE** — relaxation 이 filesystem-contained 일 때만 발화하기 때문. reviewer SOT `sandboxRelaxesCategory` (filesystem-bearing category 는 `confines.filesystem === true` 일 때만 relax) 와 동일 active-capability + 동일 truth 를 mirror. 두 production 생성 지점(`boot.ts` pluginSurfaceExecutor + `engine/conversation-loop.ts`)이 `isActiveSandboxFilesystemContained` 를 주입.
...
- **알려진 follow-up**: long-lived worker (ep-api / local-indexer 의 Python 워커) egress 를 이 shared floor 로 완전히 수렴시키는 작업이 남아 있음 — 일부는 여전히 HostApi `hostFetch` chokepoint 경유.
