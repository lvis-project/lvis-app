...
- **Gate**: `osToolSandbox` flag (Settings → 권한) 또는 `LVIS_SANDBOX_ENABLED=1`. **DEFAULT ON** (`osToolSandbox` + `hostClassifiesRisk` 둘 다 true 로 ship — shadow-mode 정합 완료 후 flip). boot 에서 한 번만 결정, runtime 변경 채널 없음. 기본 ON 이므로 boot gate (`boot/steps/sandbox-gate.ts` 의 `decideSandboxGate`) 가 **on-signal 출처**를 구분한다:
...
- **Interlock warning (degraded state 포함)**: `hostClassifiesRisk` ON 인데 OS 샌드박스가 **active 아님**(gate off, 또는 default/settings degrade 로 inactive) 이면 boot 가 LOUD warn — effect-boundary 분류가 off-hostApi mutation(direct node:fs / bare fetch / detached async frame)을 contain 못 함을 알림. degrade 경로에서도 실제 sandbox-active 상태를 기준으로 발화(`shouldWarnHostClassifyInterlock`) — 과거 `!optIn` proxy 는 default-on degrade 에서 잘못 침묵했음.
