...
- LVIS 마켓플레이스 서버 접근 권한 (사내: `https://marketplace.corp.internal`, 개발: `http://localhost:8000`)
...
```bash
cd lvis-marketplace
uv run lvm keygen --label "corp-it-publisher" --role publisher
...
| 필드 | 비고 |
|------|------|
| `$schema` | `"https://sdk.lvis.com/schemas/plugin.schema.json"` 고정 — 서버가 이 값을 보고 일반 플러그인으로 분류 |
| `id` | 마켓플레이스 카탈로그 키. 게시 후 변경 불가. flat (`agent-hub`) / 도트 (`com.ep.agent-hub`) 모두 허용 |
...
```jsonc
{
  "$schema": "https://sdk.lvis.com/schemas/mcp.schema.json",
  "id": "weather-mcp",
  "name": "Weather MCP",
  "version": "1.0.0",
  "main": "dist/server.js",
...
uv run lvm publish my-plugin-1.0.0.zip \
  --base-url https://marketplace.corp.internal \
...
- 서버 URL (예: `https://marketplace.corp.internal`)
