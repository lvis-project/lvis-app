...
6. **Pick a model from the model list.** Saving a key connects a provider; it does not move the conversation onto it. The **Model** chooser above the cards lists every connected provider's models together, grouped by provider, and picking one chooses both the provider and the model in a single step. A card whose credential you just saved says so while the conversation is still on a provider that cannot answer.
