...
`LVIS_PLUGINS_DIR=/some/path` (dev 빌드 전용) 로 override 가능 — `dev-flags.ts` 의 `@deprecated` 마킹된 escape hatch, 일반 개발 시 사용 불필요.
...
| `bun run plugins:list` 결과가 Electron registry 와 다름 (Windows 흔함) | CLI 와 Electron 모두 `~/.lvis/plugins/` 를 플러그인 루트로 사용합니다. 불일치가 발생한다면 `LVIS_PLUGINS_DIR` 환경변수가 잔존하는 것 — `unset LVIS_PLUGINS_DIR` 후 재시도. 또는 §3-4 직접 편집. |
