...
interface PluginManifest {
  // 필수 필드
  id: string;              // 패키지 식별자 (도트 형식 권장, 예: "com.ep.meeting-recorder")
...
- **도트(`.`) 형식 권장**
- 예: `com.ep.meeting-recorder`, `com.ep.email`, `page-index`
...
- **반드시 lower snake_case (`^[a-z][a-z0-9_]*$`) — 소문자·숫자·언더스코어만 허용, 대문자·도트(`.`)·하이픈(`-`) 금지**
- 예: `meeting_start`, `email_list`, `index_scan`
- 런타임이 이 값을 그대로 LLM tool name으로 사용하며 어떠한 변환도 수행하지 않음
- 로드 시 패턴 검증(`^[a-z][a-z0-9_]*$`)을 수행하며, 위반 시 플러그인 로드 거부
...
type HostPluginContext = {
  pluginId: string;          // 플러그인 ID (예: "com.ep.meeting-recorder")
...
| 구분 | 대상 | 도트 허용? | 예시 |
|------|------|-----------|------|
| 플러그인 ID | `id` 필드 | ✅ 허용 (도트 형식 권장) | `com.ep.meeting-recorder` |
...
```
// ✅ 올바른 구성
id: "com.ep.meeting-recorder"   ← 플러그인 ID, 도트 허용
