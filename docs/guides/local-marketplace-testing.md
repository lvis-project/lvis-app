...
| `LVIS_DEV=1` | ❌ | ✅ | dev 게이트 마스터 — DevTools / hot-reload / 테스트 키 자동 포함 |
| `LVIS_DEV_RELOAD=1` | ❌ | ❌ (수동) | dist/ watch + reloadPlugin |

**마켓플레이스 zip envelope 검증을 dev 키로 통과시키려면**:

```bash
# 옵션 A — bun run start 경유 (수동 env 추가):
LVIS_ALLOW_TEST_MARKETPLACE_KEYS=1 bun run start

# 옵션 B — bun run dev (LVIS_DEV=1 자동, 테스트 키도 자동 포함됨):
bun run dev
```

`testMarketplaceKeysAllowed()` (`src/boot/dev-flags.ts:88`) 는 `LVIS_ALLOW_TEST_MARKETPLACE_KEYS=1` **또는** `LVIS_DEV=1` 이 켜져 있으면 trust set 에 `dev-v1` / `poc-v1` 등 테스트 키 포함합니다.
...
| `signature verification failed` (앱 install 시) | (a) `LVIS_ALLOW_TEST_MARKETPLACE_KEYS=1` 누락 (`bun run start` 시) 또는 `bun run dev` 미사용. (b) 서버 `MARKETPLACE_SIGNING_PRIVATE_KEY_*` 키 ID 와 lvis-app trust set (SDK `MARKETPLACE_PUBLIC_KEYS`) 가 짝 안 맞음. |
