...
the LVIS marketplace (typically the corporate IT Platform org). Each publisher owns
one or more **ed25519 signing keypairs**:

- **Private key** — lives only in the corporate IT secrets vault and is injected
...
into the corporate IT secrets vault; paste the public key into `publisher-keys.ts`.
...
- **Private key**: corporate IT secrets vault (production) / GitHub Actions org
