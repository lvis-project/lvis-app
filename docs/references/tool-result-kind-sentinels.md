# Tool Result Kind Sentinels

This is the English default page for `docs/references/tool-result-kind-sentinels.md`. The Korean source document is preserved at [docs/ko/references/tool-result-kind-sentinels.md ](../ko/references/tool-result-kind-sentinels.md).

## Status

- Category: references
- Default language: English
- Korean mirror: ko/references/tool-result-kind-sentinels.md

## Summary

This page keeps the canonical documentation path English-first while retaining the original Korean document at the same mirrored path below `docs/ko`. Update this page with the English canonical content as the topic evolves; keep the Korean mirror linked when Korean-specific background or review history is still useful.
