...
interface PluginManifestV2 {
  id: string;                  // dot 형식: "com.ep.meeting-recorder"
  name: string;
  version: string;
...
- 플러그인 ID는 dot 형식 유지: `com.ep.meeting-recorder`
...
| 플러그인 | ID | 주요 methods |
|---------|-----|-------------|
| Meeting Recorder | `com.ep.meeting-recorder` | `meeting_start`, `meeting_stop`, `meeting_summarize` |
