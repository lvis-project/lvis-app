...
interface PluginManifest {
  /** 플러그인 고유 식별자. dot 형식: "com.ep.meeting-recorder" */
  id: string;
  name: string;
  version: string;
...
### 5.1 Meeting Recorder (`com.ep.meeting-recorder`)
  ...
  "index_add_folder": {
    ...
    "inputSchema": {
      ...
      "properties": {
        "folder": {
          "type": "string",
          "description": "인덱싱할 폴더 경로. 예: /Users/example/Documents 또는 ~/Documents"
        }
      }
    }
  }
...
- **플러그인 ID**: dot 형식 (`com.ep.meeting-recorder`). LLM에 노출 안 됨.
...
| 서명 검증 | Corporate Internal Root CA 필수 | 정책 설정에 따라 |
