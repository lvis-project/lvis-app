...
- `meeting`, `calendar`, `email`, `index`, `task`, `briefing` → public. 조용히 허용. (`task.*` namespace 는 `lvis-plugin-tasks` 가 owner — Phase 3 부터 `task.created` / `task.updated` / `task.completed` / `task.deleted` / `task.deadline.approaching` / `task.deadline.overdue` 발행. tasks-plugin-split 참조.)
...
| `addTask(task)` | 액션 아이템 → LVIS 태스크 자동 생성. **Phase 2 이후**: `lvis-plugin-tasks` 가 owner — `addTask` 는 `callTool("task_add", …)` 로 위임되는 thin shim 이 된다. 호출자(meeting/email) 변경 없음. | UI 직접 조작 대체 |
