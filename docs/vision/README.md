...
- [ ] internal search 기반 업무 자동화 (출장·품의·보고 등 반복 업무)
- [ ] 에이전트 투 에이전트(A2A) 협업 구조 구현
- [ ] 에이전트 허브: 다양한 업무 에이전트 연결 및 조율
- [ ] 기업 프로세스(internal search 등)와의 통합
...
| Phase 3 | 퍼스널 에이전트 시스템 프로토타입 (Electron + Rust + Internal search) | ⏳ 예정 |
| Phase 4 | internal search 연동 및 기업 업무 자동화 | ⏳ 예정 |
