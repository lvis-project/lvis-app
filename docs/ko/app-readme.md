...
Registry CLI는 실행 중인 호스트의 live state와 durable state가 어긋나지 않도록 조회 전용입니다. 설치와 상태 변경은 마켓플레이스 카드, 호스트 설정, `lvis://install/<slug>` 딥링크, 또는 `lvis-cli install file://<path-to-dist.zip>` 사이드로드를 사용합니다.
