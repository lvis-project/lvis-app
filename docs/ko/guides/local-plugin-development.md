...
1. **trusted publisher key 로 서명된 zip 을 직접 받아 설치**: 마켓플레이스 외 채널(예: enterprise 파일 공유)로 받은 서명된 artifact 를 (앱이 직접 install API 를 노출하지 않으므로) registry 에 수동 추가하는 방법은 사실상 없음 — 정상 경로는 마켓플레이스 install.
