...
매니페스트 작성 규칙(필수 필드, AJV 검증, tool 스키마 — v6 는 pure `Tool[]`)은 [현재 English Plugin Development Guide](../../guides/plugin-development.md#manifest-basics)와 [`plugin-manifest.schema.json`](../../../schemas/plugin-manifest.schema.json)을 참고하세요. 한국어 [`plugin-development.md`](./plugin-development.md)는 과거 검토·논의 이력이며 현재 계약의 정본이 아닙니다.
...
`bun run start`/`bun run dev`의 현재 런처 계약:

| env | 효과 |
|-----|------|
| `LVIS_DEV=1` | 개발자 도구와 개발 모드를 활성화. `bun run dev`가 자동 설정 |
| `LVIS_DEV_RELOAD=1` | 설치된 플러그인 payload의 변경 감시와 reload 활성화 |
| `LVIS_DEV_CONSOLE=0` | DevTools 자동 열기 끄기. `bun run start`의 기본값 |
| `LVIS_WIN_NO_SANDBOX=1` (Windows) | unpackaged corp/VDI 실행에서만 Chromium sandbox 우회. Windows safe launcher가 필요할 때 설정 |
...
> `bun run start` 는 **`LVIS_DEV=1` 을 세팅하지 않습니다**. 개발자 설치 UI가 필요하면 `bun run dev` (`run-electron-dev.mjs`)를 사용하세요. `LVIS_DEV_RELOAD` (§5)는 독립적으로 개발 모드를 엽니다. 어떤 dev 플래그도 marketplace envelope 검증, 설치 receipt 무결성, entry 경로 containment를 우회하지 않습니다. 마켓플레이스 trust root는 앱 호스트가 소유하며 SDK는 타입/소스 계약만 제공합니다.
...
| 게이트 | 동작 |
|--------|------|
| 모든 `LVIS_DEV*` env | packaged 빌드에서 무시하고 부팅 시 stale/tampered 플래그를 감사 로그에 기록 |
| Marketplace envelope | Host 내장 publisher trust set으로 항상 검증하며 dev 우회 없음 |
| 설치 payload | 설치 receipt와 payload digest를 항상 검증 |
| Entry 경로 | `pluginsRoot/<id>/` containment를 항상 강제하며 linked entry 우회 없음 |
...
| 설치 무결성 검증 실패 (dev) | Settings → Plugin Config → 개발자 도구에서 현재 빌드 폴더를 다시 설치해 receipt를 재생성. 검증 우회 env는 없음 |
