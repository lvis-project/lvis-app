...
> `_meta.ui.visibility`, filesystem path 판정은 `_meta["xyz.lvis/pathFields"]`,
...
2. **SDK schema first** — 모든 `tools[]` 항목은 pure MCP `Tool` 객체로 `name`, `description`, `inputSchema`, `_meta?` 를 선언한다. path 판정은 `_meta["xyz.lvis/pathFields"]` 가 SOT 이고, per-tool category 는 manifest 필드가 아니라 host 가 invocation 별로 분류한다 (플러그인이 자기 위험도를 스스로 매기지 않는다).
...
3. **도구 이름 제한 없음** — 어떤 접미사든 `"app"` visibility 로 노출할 수 있다. 실제 위험 작업은 이름이 아니라 `_meta["xyz.lvis/pathFields"]` 와 host-classified category 로 평가한다.
...
각 tool 객체는 `inputSchema` (JSON Schema draft-07) 를 담는다. filesystem path 인자는 `_meta["xyz.lvis/pathFields"]` (Layer 0/1/5 path 검사 입력) 로 선언하고, per-tool category 는 manifest 필드가 아니라 host 가 invocation 별로 분류한다. top-level 은 반드시 `"type": "object"` — 모든 LLM vendor 공통 요구사항. (pre-v6 의 `toolSchemas` map 은 #885 Phase R 에서 tool 객체로 인라인됨.)
...
- [ ] 파일 경로 인자가 있으면 `_meta["xyz.lvis/pathFields"]` 에 dotted selector 로 선언 (per-tool category 는 host 가 분류 — manifest 에 선언 안 함)
...
- 각 tool `inputSchema` 필수: `index_add_folder` 는 path 인자를 `_meta["xyz.lvis/pathFields"]` 로 선언한다 (effective category 는 host 가 분류).
...
- [ ] 각 `tools[]` Tool 객체에 `description` (10자 이상), `inputSchema`, `_meta.ui.visibility`, 필요 시 `_meta["xyz.lvis/pathFields"]`
