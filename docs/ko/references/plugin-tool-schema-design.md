...
1. **Capability gate = HostApi + 선언적 capability** — HostApi 자체가 1차 게이트이며, `capabilities[]`/`uiActions` 는 MS Graph · 이벤트 namespace · renderer IPC allowlist 등 HostApi 단독으로 표현 불가능한 2차 게이트를 보강한다.
...
| `uiActions` | `PluginRuntime.callFromUi()` allowlist | renderer IPC 호출 |
...
Renderer UI 는 `lvis:plugins:call` IPC 를 통해 allowlist 된 플러그인 runtime method 호출을 요청할 수 있다. 이 IPC 는 handler 직접 호출 경로가 아니며, `PluginRuntime.callFromUi()` 가 매번 `manifest.uiActions` 를 재확인한다. `tools[]` 는 LLM-facing surface, `uiActions` 는 renderer IPC surface 이며, UI 전용 메서드는 `uiActions` 에만 선언할 수 있다. `tools[]` 에도 있는 UI action 은 ToolExecutor 권한 경로를 통과하고, UI-only action 은 auth status polling 을 제외하고 preload 가 확인한 active user activation 이 있어야 handler 로 전달된다.

**규칙:**

1. **surface 분리** — `tools[]` 는 LLM 카탈로그, `uiActions` 는 renderer IPC allowlist 이다. 같은 이름을 양쪽에 둘 수 있지만, auth/status/login/panel-internal 메서드는 `uiActions` 전용으로 둘 수 있다.
2. **권한 경로 분리** — `PluginRuntime.callFromUi(method, payload)` 는 매번 `manifest.uiActions` 를 재확인한 뒤 host delegate 로 넘긴다. 이름이 `tools[]` 에도 있으면 ToolExecutor 를 호출하므로 sensitive path, allowed directories, reviewer, hooks, audit 정책이 적용된다. `tools[]` 에 없는 UI-only action 은 auth status polling 을 제외하고 active user activation 이 있어야 실행되며, runtime 이 allowlist 재검증과 audit log 를 남긴다.
3. **도구 이름 제한 없음** — 어떤 접미사든 `uiActions` 에 넣을 수 있다. 실제 위험 작업은 이름이 아니라 SDK manifest `category/pathFields` 와 host 권한 정책으로 평가한다.
...
"uiActions": {
  "msgraph_email_reply": {},
  "msgraph_calendar_delete": {},
  "index_remove_folder": {}
}

// OK — UI 전용 메서드. LLM에는 노출되지 않음
"uiActions": {
  "meeting_push_chunk": {}
}
```
...
```jsonc
{
  ...
  "auth": {
    ...
    "logoutTool": "msgraph_signout"  // optional, uiActions 안
  }
}
```

**Cross-field 강제** — `manifest-validation.ts` (§B-3 와 같은 hand-rolled 위치) 가 `auth.{statusTool,loginTool,logoutTool} ∈ uiActions` 이고 `auth.*Tool ∉ tools[]` 임을 load-time 에 검증. AJV 단독으로는 cross-object membership 표현 불가.
...
[manifest:meeting] schema validation failed (/path/to/plugin.json): /uiActions/meeting.start must match pattern "^[a-zA-Z_][a-zA-Z0-9_]*$"
...
function App({ bridge, rootEl }: { bridge: PluginBridge; rootEl?: HTMLElement }) {
  useTheme(bridge, {
...
- **Renderer UI → plugin method**: 호스트가 제공하는 generic 핸들러 `lvis:plugins:call(toolName, payload)` 단 하나 경유. `manifest.uiActions` allowlist 로 gating (§2.2) — 이름 패턴 차단은 없으며, 확인 UX 는 플러그인이 자체 UI 로 구현한다. 플러그인이 채널을 직접 선언하지 않는다.
...
- `uiActions: { "meeting_transcript": {}, "meeting_sessions": {} }` — 렌더러에서 호출 가능한 조회용 메서드를 노출. 같은 이름이 `tools[]` 에도 있으면 host delegate 를 통해 ToolExecutor 권한 경로를 통과하고, UI-only 메서드는 active user activation gate 와 runtime allowlist/audit path 를 통과한다.
...
- 메일/캘린더 mutation tool 은 `toolSchemas` 에 `category: "write" | "network"` 를 명시하고, UI 호출이 필요하면 `uiActions` 에 선언한다.
...
| **`permissions[]` 선언형 필드** | ⚠️ **부분 승인** | full permission 문자열 배열은 기각되었지만, 다음 3종의 선언형 게이트가 대체 도입됨: (1) `uiActions` — renderer→plugin allowlist (`tools[]` 와 별도 surface, 파괴적 도구의 확인 UX 는 플러그인이 책임), (2) `capabilities[]` — `ms-graph-consumer` HostApi gate + event-emit namespace gate, (3) `PLUGIN_PRIVATE_NAMESPACES` — subscription deny-list. §2.2, §2.3. |
