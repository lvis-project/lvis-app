...
**Phase 1.5 구현 주의사항**:
- `pluginMarketplace.uninstall()`은 npm uninstall + registry 파일 갱신만 담당. SDK v12 이후 keyword routing은 존재하지 않으며, `PluginRuntime.stopAll()`/restart 시점에는 활성 플러그인의 런타임과 등록된 Tool 노출만 재정리.
