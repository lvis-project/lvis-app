...
| 원칙 | 적용 |
|---|---|
| **Fail-safe defaults** | Manifest 의 tool 은 pure MCP `Tool[]` 이다 (#885 Phase R). filesystem path 판정 SOT 는 tool 객체의 `_meta["lvisai/pathFields"]` 이며, per-tool category 는 manifest 필드가 아니라 host 가 invocation 별로 분류한다. host 는 plugin 의 `isReadOnly()` 는 신뢰하지 않는다. `fallbackOnError` enum: `deny | rule` 만 (allow-and-audit 폐지), 기본값은 `deny` |
...
| **User-in-the-loop > silent** | Headless 의 implicit allow 폐지. Reviewer agent 는 foreground LOW 만 auto+audit, foreground MED/HIGH 는 main-owned in-flow approval dock, headless MED/HIGH 는 deferred queue 로 처리한다. Chat 문장이나 LLM 출력은 승인 권한이 아니다. LLM-free `rule` path 도 같은 verdict semantics 를 따른다. |
...
| Plugin | Manifest `tools: Tool[]` → `plugin-tool-adapter.ts` | 위와 동일. path 판정은 tool 의 `_meta["lvisai/pathFields"]` 가 SOT 이고 effective category 는 host 가 invocation 별로 분류한다. plugin code 의 `isReadOnly()` 나 tool-name 추론으로 보정하지 않는다. |
...
│   ├ permissions.additionalDirectories[] (LVIS scope naming) │
...
**`pathFields[]` declaration:** native host tools and plugin tools declare path-typed input fields through the `Tool` path-fields contract. `extractTargetFilePaths` scans declared fields only, not every string field. Plugin path fields come from the tool object's `_meta["lvisai/pathFields"]` (the sole LVIS-proprietary manifest key, #885 Phase R), support dotted selectors, and are used by Layer 0/1 and the reviewer.
...
(`permissions.additionalDirectories` 는 사용자가 기존 working directory 외에
추가 허용 범위를 명시한다는 의미를 그대로 드러내는 LVIS 설정명이다.
`allowedDirectories` alias/shim 은 두지 않는다.)
...
**Runtime mode semantics:** `default` 는 read 허용 + write/shell/network 및 `decisionOverride: "ask"` builtin meta를 ask 한다. `strict` 는 read 포함 모든 도구 실행을 ask. `auto` 는 user-visible write/shell/network 및 `decisionOverride: "ask"` builtin meta를 같은 foreground reviewer lane으로 보내고, 설정된 foreground 자동승인 임계값(low/medium; off면 기존 explicit approval 경로)까지 allow+audit, 그 초과/HIGH 는 main-owned in-flow approval dock 에서 사용자 결정을 받는다. Chat 문장 감지나 executor 재시도 메모리는 승인 권한으로 사용하지 않는다. `auto` headless mutation 은 reviewer/deferred queue 로 보낸다. `allow` 는 명시적 전체허용 opt-in 이며 Layer 0 sensitive path, Layer 1 directory scope, deny rules, overlay-trigger-origin mutation guard 는 우회하지 않는다.
...
**Manifest validation:** per-tool category 는 manifest 필드가 아니다 (#885 Phase R) — host 가 invocation 별로 effective category 를 분류한다. path 판정 SOT 는 tool 객체의 `_meta["lvisai/pathFields"]` 이며, host 는 app-local extension/name inference 없이 pure `Tool[]` manifest 를 등록한다.
...
| builtin × write | L0/L1 → ask | L0/L1 → ask | L0/L1 → reviewer LOW allow + audit / MED-HIGH approval dock | L0/L1 → reviewer (L5) | L0/L1 → allow + audit |
| builtin × shell | L0/L1 → ask + AST | L0/L1 → ask + AST | L0/L1 → reviewer LOW allow + AST + audit / MED-HIGH approval dock + AST | L0/L1 → reviewer | L0/L1 → allow + AST |
| builtin × network | L0/L1 → ask + endpoint | L0/L1 → ask | L0/L1 → reviewer LOW allow + audit / MED-HIGH approval dock | L0/L1 → reviewer (L5) | L0/L1 → allow + audit |
| plugin × read | L0/L1/L4 → allow | L0/L1/L4 → ask | L0/L1/L4 → allow | L0/L1/L4 → allow, reviewer if out-of-dir | L0/L1/L4 → allow |
| plugin × write | L0/L1/L4 → ask | L0/L1/L4 → ask | L0/L1/L4 → reviewer LOW allow + audit / MED-HIGH approval dock | L0/L1/L4 → reviewer | L0/L1/L4 → allow + audit |
| plugin × shell | L0/L1/L4 → ask + AST | L0/L1/L4 → ask + AST | L0/L1/L4 → reviewer LOW allow + AST + audit / MED-HIGH approval dock + AST | reviewer | L0/L1/L4 → allow + AST |
| plugin × network | L0/L1/L4 → ask + endpoint | L0/L1/L4 → ask | L0/L1/L4 → reviewer LOW allow + audit / MED-HIGH approval dock | reviewer | L0/L1/L4 → allow + audit |
...
```jsonc
{
  "permissions": {
    "reviewer": {
      ...
      // explicit approval, "low" = 리뷰어 LOW 시 dock 없이 통과. MED/HIGH 는
      // main-owned in-flow approval dock 에서 사용자 결정을 받음.
...
open the main-owned in-flow approval dock. The dock decision is bound to the
...
preparation retains the legacy batch and ordinary approval-dock flow. The
enabled path keeps one rationale-only round, records generation outcome
separately from reviewer reevaluation, routes either failure to the same dock
...
| Source | Public metadata | LVIS use |
|---|---|---|
| [OpenHarness](https://github.com/HKUDS/OpenHarness) | MIT | Only files with explicit OpenHarness headers may reuse implementation material. |
| [Paperclip](https://github.com/paperclipai/paperclip) | MIT | Agent control-plane UX/API comparison only. |
| [OpenCode](https://github.com/sst/opencode) | MIT | UX/API behavior comparison only. |
| [Cline](https://github.com/cline/cline) | Apache-2.0 | UX/API behavior comparison only. |
| [Gemini CLI](https://github.com/google-gemini/gemini-cli) | Apache-2.0 | UX/API behavior comparison only. |
| [OpenAI Codex CLI](https://github.com/openai/codex) | Apache-2.0 | Approval/sandbox behavior comparison only. |
| [Hermes Agent](https://github.com/NousResearch/hermes-agent) | MIT | Command-surface behavior comparison only. |
| [Claude Code legal/compliance](https://docs.claude.com/en/docs/claude-code/legal-and-compliance) | Anthropic terms, not OSS | Public UX reference only; never an implementation source. |
| Leak-derived Claude Code mirrors | Non-official / excluded | Must not be used as implementation, naming, or architecture source. |
...
| 영역 | 현재 구현 | SOT / 파일 |
|---|---|---|
| Tool category contract | Host registry 는 `read/write/shell/network/meta`; per-tool category 는 manifest 필드가 아니라 host 가 invocation 별로 분류한다 (#885 Phase R). plugin path 판정은 tool 객체의 `_meta["lvisai/pathFields"]` 가 SOT 이며 app-local name inference 나 compatibility grace 는 없다. | `src/permissions/category-registry.ts`, `src/plugins/runtime/manifest-validation.ts` |
...
- (당시) SDK schema 는 `toolSchemas[*].category` 와 `toolSchemas[*].pathFields` 를 제공했다 — #885 Phase R 에서 pure `Tool[]` + `_meta["lvisai/pathFields"]` 로 대체됨 (legacy).
...
15. `additionalDirectories` naming (LVIS permission scope naming)
...
- Sandbox-write auto-LOW (P1): the manifest self-attestation field `toolSchemas[*].writesToOwnSandbox?` was **removed in #885 Phase R** — containment is now host-derived from the worker effect-boundary, not a plugin declaration. When the write's resolved `pathFields` value canonically stays inside `~/.lvis/plugins/<ownerPluginId>/`, the reviewer auto-LOWs the write. Sound-by-construction — the runtime verifies the path-containment claim, the declaration alone is insufficient. Without this rule plugins like ms-graph that write MSAL token cache to their own sandbox get caught by the "write outside allowed dirs" HIGH rule because the host's `allowedDirectories` does not include plugin sandboxes by design (§5 file-based memory).
