...
사용자는 전체 응답 완료를 기다리지 않고 **입력 분류 → reasoning → tool group → reasoning → assistant** 순서를 스트림으로 확인한다. provider별 내부 transport는 다를 수 있지만, renderer가 받는 표준 이벤트는 `reasoning_delta`, `text_delta`, `tool_start`, `tool_end`, `assistant_round`, `done`이다.

```mermaid
sequenceDiagram
    participant UI as Renderer
    participant IPC as IPC Bridge
    participant CLASSIFY as Input Classifier
...
| 항목 | 설명 |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| **초기 입력 분류** | `InputClassifier`는 `runTurn()` 초입에서 호스트 slash command만 분류한다. plugin keyword/skill route는 없다. |
...
| ------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| User message creation | renderer input → `ipcMain.handle("lvis:chat:send")` | Electron IPC + command-only input classification |
...
| 2    | 에이전트 멘션 | "@이영희 이거 확인해줘" | ConversationLoop에서 Agent Hub Tool 사용   |
| 3    | 의도 + 엔티티 | "출장 품의 작성해줘"    | Route Engine → the LLM backend + 관련 도구 |
| 4    | 일반 대화     | "안녕하세요"            | the LLM backend 직접 세션                  |
...
- `onActiveStateChange(pluginId, enabled)` — 사용자 활성/비활성 토글에서만 발화하며 runtime unload가 아니다. 비활성화 시 호스트는 ConversationLoop transient scope만 정리하고 ToolRegistry는 유지한다. 따라서 auth/config/UI 호출은 계속 runtime 도구를 실행할 수 있고, 모델 노출만 ConversationLoop tool scope에서 차단된다. 재활성화 시 Host가 현재 plugin scope를 다시 계산한다.
