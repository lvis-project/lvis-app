...
graph LR
    subgraph P1["🖥️ LVIS Client"]
        direction TB
        P1A["Electron Shell"]
        P1B["Core Engines<br/>(Input·Route·Index)"]
        P1C["Plugin Host"]
...
sequenceDiagram
    ...
    PluginMgr->>App: 5b. Register plugin UI slots + Skills + Tools + Hooks + MCP servers
    App->>App: 6. Initialize Core Engines (Input · Route · Tool · Index · Memory)
...
이 섹션은 **사용자 입력을 어떤 실행 경로로 보낼지**를 다룬다.
`InputClassifier`는 호스트 소유 slash command만 분류한다. 자연어 키워드는
플러그인 scope를 활성화하거나 Tool schema를 preload하지 않는다. Bundled instruction
discovery는 `manifest.skills`, Tool 발견은 호스트가 선택한
plugin scope와 `tool_search`가 담당한다. 실제 턴 내부의 reasoning → tool
...
flowchart TB
    ...
    CLASSIFY["InputClassifier<br/>(host slash command only)"]
    ROUTE{"RouteEngine"}

    COMMAND["/command 직접 실행"]
    CHAT["ConversationLoop.runTurn()<br/>일반 대화 / 도구 루프"]

    INPUT --> MEMORY --> CLASSIFY --> ROUTE
...
flowchart TB
    ...
    CLASSIFY["InputClassifier.classify()<br/>(host slash command only)"]
    ...
    USER --> IPC --> CLASSIFY --> ROUTE --> LOOP --> HISTORY --> PROMPT --> PROVIDER --> REASON --> TOOL_CHECK
    TOOL_CHECK -->|"Yes"| POLICY_CHK -->|"허용"| PERM --> TOOL_EXEC --> TOOL_RESULT --> PROVIDER
    TOOL_CHECK -->|"No"| ROUND --> POST --> SAVE
    POLICY_CHK -->|"차단"| TOOL_RESULT
```

> **문서 원칙**: 이 섹션은 `ConversationLoop`의 현재 구현을 기준으로 설명하되, 아직 미구현인 목표 설계(Governance sync, richer session history)도 함께 유지한다.
...
| **2. 입력 분류** | `InputClassifier.classify()` | 대화 루프 본문 전에 호스트 slash command 여부를 판단 | imported plugin/MCP app envelope는 command로 해석하지 않음                                                                                                                                        |
| **3. 실행 경로 결정** | `RouteEngine.route()` | `command` 또는 `llm` 경로로 확정 | 플러그인 keyword/skill route 없음                                                                                                                                                                 |
...
사용자는 전체 응답 완료를 기다리지 않고 **input classification → reasoning → tool group → reasoning → assistant** 순서를 스트림으로 확인한다. provider별 내부 transport는 다를 수 있지만, renderer가 받는 표준 이벤트는 `reasoning_delta`, `text_delta`, `tool_start`, `tool_end`, `assistant_round`, `done`이다.

```mermaid
sequenceDiagram
    participant UI as Renderer
    participant IPC as IPC Bridge
    participant KW as Input Classifier
...
| 항목 | 설명 |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| **초기 사전판단** | `InputClassifier`는 Host 소유 slash command만 분류한다. 자연어 Tool 선택은 model-visible Tool과 `tool_search`가 담당한다. |
...
LVIS 는 라이선스와 출처가 확인된 공개 OSS/공식 문서에서 관찰 가능한 컨텍스트 관리 패턴과 자체 요구사항을 합성한 compact pipeline 을 채택한다. 토큰 preflight 가 임계치를 넘으면 LLM 호출을 잠시 차단하고, _현재 세션 안에서_ tool-result stubbing 과 LLM compact 를 실행해 실제 토큰을 줄인다.

**사용자 contract**: compact 는 어떤 input shape (단일 200K 메시지 / 다수 중간 메시지 누적 / 깊은 tool chain) 에서도 reduce 보장. 수동 메시지 삭제 없이 항상 성공해야 함.

**Tool-Result Stubbing (preventive, marker-only)**: 매 post-turn 마다 실행. `markStaleToolResults()` 가 오래된 `tool_result` 메시지에 `meta.compactedAt` 단일 마커만 set — 메모리상의 content 는 _verbatim 보존_ 된다. 마커가 있는 메시지는 `wire-serialize.ts:stubMarkedToolResults` 가 provider 호출 직전 + `saveSession` 직전에 stub 으로 변환. 최근 N개 (기본 8) raw 유지, 200자 미만 제외 (LVIS noise-floor policy), `toolUseId` 참조 무결성 보존 (`src/engine/auto-compact.ts:114-124`).
...
| ------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| User message creation | renderer input → `ipcMain.handle("lvis:chat:send")` | Electron IPC + Host command classification |
...
해당 compactNum 위치에서 _명시적으로_ 새 세션을 fork 한다. 자동 fork 는 일어나지 않는다.
...
**Prompt-injection fence**: rolling summary preamble 을 system prompt 에 주입할 때 `<prior-context-summary>` 블록을 _명령 해석 금지_ fence 로 wrap (system-prompt-builder Section 8). 이전 컨텍스트의 사용자 입력이 요약을 거쳐 다음 턴 / fork 된 자식 세션의 system prompt 로 흘러들어가는 vector 차단.
...
│   ├── input-classifier.ts, route-engine.ts,
│   │   tool-registry.ts
│   └── network-guard.ts
│
...
### 6.1 Input Classification & Tool Discovery

`InputClassifier`는 Host가 소유한 slash command와 일반 입력만 구분한다.
플러그인 Skill은 instruction bundle이며 keyword router가 아니다. 자연어
Tool 선택은 model-visible Tool inventory와 bounded `tool_search`를 통해
모델이 수행한다.

```mermaid
flowchart LR
    INPUT["Raw Input"] --> CLASSIFY{"Host slash command?"}
    CLASSIFY -->|"Yes"| CMD_EXEC["Command Executor"]
    CLASSIFY -->|"No"| ROUTE_ENGINE["Agent Route Engine"]
    SKILLS["Bundled manifest.skills<br/>instructions"] --> MODEL["Model"]
    CATALOG["Active Tool catalog"] --> SEARCH["bounded tool_search"]
    SEARCH --> MODEL
    ROUTE_ENGINE --> MODEL
```
...
| 2    | 앱/플러그인 envelope | Host가 검증한 app/overlay 입력 | provenance를 보존해 ConversationLoop 진입 |
| 3    | 일반 대화     | "출장 품의 작성해줘"    | 모델이 Skill 지침과 Tool catalog를 사용 |

**확장 설계 노트**

- 플러그인은 `manifest.skills`로 instruction bundle을 선언한다.
- Tool은 host-selected plugin scope 안에서 eager exposure 또는 `tool_search`
  discovery로 모델에 제공된다.
...
> §8 ApprovalGate 의 입력으로 흐른다 — 두 섹션을 _중복 승인 단계_ 로
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------- |
| Plugin tool metadata | SDK manifest schema 의 per-tool `pathFields` 가 filesystem path 판정 metadata 의 SOT 이다. `category` 는 선택적 provider metadata 이며 effective category 는 host-side `inspectHostRisk`/executor policy 가 invocation 별로 산출한다. `meta` 및 향후 host-only category 는 plugin contract 로 자동 확장되지 않는다. | 추가 plugin 도입 시 SDK schema `pathFields` / `workerId` 선언과 plugin sanity test 를 PR merge gate 로 유지 |
...
**Sandbox-write containment (issue #664 P1):** plugin 의 write 가 자기 sandbox
안에만 있는지 판정하는 로직. **manifest self-attestation 필드
`toolSchemas[*].writesToOwnSandbox?` 는 #885 Phase R 에서 제거**되었고, 이제
host 가 worker effect-boundary(sandbox filesystem containment)로 판정한다.
path field 값이 실제로 `~/.lvis/plugins/<pluginId>/` 안에 있으면 LOW 로
...
이후로 deferred. v1 의 `allow` 는 _additional approval signal_ — Layer
...
Manifest 는 각 tool 의 `_meta["lvisai/pathFields"]` 로 filesystem path 판정
metadata 를 선언한다 (per-tool category / workerId 는 manifest 필드가 아님 —
...
SDK schema/types 와 active plugin manifests 는 `pathFields` 를 선언한다.
`category` 가 있으면 provider metadata 로만 취급하고, read/write 판정은
host-observed risk/effect SOT 에서 계산한다. 향후 sandboxed runtime 에서
fs proxy 를 더 강하게 적용한다. 호환 shim 이나 boot-warn grace 를 두지
않는다.
...
(write 차단). compromised tool 이 _새 entry_ 추가는 막지만 기존 log
rewrite 불가능.
...
Layer 0–8 의 permission policy 는 _어떤 도구 호출을 허용/거부_ 하는지를
...
- **Network floor (shared, deny-by-default)**: ASRT 의 loopback proxy network
  floor 는 _모듈-레벨 SHARED config_ 를 읽으므로, egress 는
  boot 에서 `strictAllowlist: true` + 모든 로드된 plugin manifest 의
  `networkAccess.allowedDomains` UNION 을 SHARED config 로 설정해 강제한다
  (loopback proxy 가 strict-union 바닥). 이는 _per-worker_ 격리가 아니라
  UNION allow-list 다 — sandboxed process 는 _임의의_ 로드된 plugin 이 선언한
...
- **Gate**: `osToolSandbox` feature flag (Settings → 권한) 또는
  `LVIS_SANDBOX_ENABLED=1`. **STAGED default (macOS-first)** (PR #1390 의
  default-ON 을 단계화): `osToolSandbox` 기본값 = **`darwin` ON / `linux`+`win32`
  OFF(opt-in)** — live-verified-active 인 macOS 만 우선 ON, Linux/Windows 는
  C/D-series 샌드박스 QA 가 green 될 때까지 Settings 토글로 opt-in. _Convergence
  plan_: C/D QA 통과 시 `src/data/settings-store.ts` 의
  `osToolSandbox: process.platform === "darwin"` 단일 표현식을 `true` 로 flip.
  `hostClassifiesRisk` 는 **전 플랫폼 default ON** — non-sandbox 플랫폼에서도
  안전한 이유는 read-relaxation 이 sandbox-active 와 커플링되어(아래) fallback
  하기 때문. 활성 불가 시 동작은 on-signal 출처로 갈린다:
  **EXPLICIT** (`LVIS_SANDBOX_ENABLED=1`, 명시적 power-user/CI 강제) +
  mac/linux deps-missing → boot **fail-closed abort** (no-fallback 보존);
  **DEFAULT/settings-on** (env 없이 기본값/토글로 ON) → **graceful degrade**
  (loud `log.warn` + `isAsrtSandboxActive()=false` + boot 계속, non-bricking).
  Windows (srt-win 미설치) 는 explicit 이어도 항상 degrade (first-run brick
  방지). 결정은 `src/boot/steps/sandbox-gate.ts` `decideSandboxGate(
  skip|activate|degrade|abort)` 한 곳. host-classify↔sandbox interlock 경고는
  실제 `sandboxActive` 상태에서 발화(degrade 경로 포함). Gate 는 boot 에서
  _정확히 한 번_ 결정되며 runtime 변경 채널은 없다.
- **Read-relaxation ↔ plugin effect-boundary FILESYSTEM-CONTAINMENT 커플링
  (confines-aware)**: foreground plugin read-relaxation
  (`src/tools/invocation-authorization.ts`) 은 `hostClassifiesRisk` ON **AND
  `sandboxFsContainedProvider(tool)` (= `isActiveSandboxFilesystemContainedForPluginEffects(tool)`)
...
- **Activation telemetry**: boot 의 gate 는 **boot 당 1건** 의 구조화 audit
  ...
  가 전용 채널 `~/.lvis/audit/<date>.sandbox-gate.jsonl` 에 append (hand-rolled
  file write 없이 AuditLogger 경유). Linux/Windows 기본값 flip 전 실세계
...
- **Windows (srt-win)**: 동일 gate 에 합류한다 (`LVIS_SANDBOX_WINDOWS` 별도
  ...
  **filesystem+network partial** 샌드박스다. process 격리는 제공하지 않는다.
  ...
  활성화는 _오직_ 명시적 consent 버튼이 호출하는 1회 self-elevating UAC 설치만
...
- **Windows 설치 consent UX**: Settings → 권한 의 "OS 도구 샌드박스"
  ...
  의 **단일 self-elevating UAC** 를 트리거하는 _유일한_ privilege-escalation
...
- **Substrate-aware reviewer capability (필수 불변식)**: process-global
  ...
  ON 이 비격리 plugin/MCP 도구의 승인 게이트를 _완화_ 하던 역효과를 차단한다.
...
7개 차단 패턴과 `warn` / `deny` 모드를 분리해 `lvis-app/src/tools/invocation-runner.ts` Step 2.5에서 실행한다.

| Pattern ID | 정규식 패턴 | 차단 사유 |
| ------------------ | ------------------------------------------ | ------------------------------------------- | ----------------------- |
| `rm-rf-root` | `\brm\s+(-[rfRF]+\s+)+(\/\|~\|\$HOME\|\*)` | `rm -rf` 위험 경로 (`/`, `~`, `$HOME`, `*`) |
| `curl-pipe-sh` | `\b(curl\|wget\|fetch)\b[^|]_\|\s_(sh\|bash\|zsh\|fish)` | 원격 스크립트 직접 실행 |
...
payload (`lastThemePayload`) 를 _명시적으로 요청_. 받은 토큰을
...
> **§6.3 Permission Policy 와의 분리:** §6.3 은 _개별 도구 호출_ 에 대한
> Layer 0–9 평가 (sensitive paths, allowed dirs, category × source ×
> mode, reviewer agent, hook chain) 를 다룬다. 본 §8 은 _에이전트
> 행위 전체_ 에 대한 사용자 승인 모델 (자율 게시 / 외부 송신 / 업무
> 일지 공개 범위) 을 다룬다. permission policy Layer 3 의 `network = ask + endpoint`
> 결정은 §8 ApprovalGate 의 입력으로 흐르므로 두 섹션이 _중복 승인
> 단계_ 처럼 보일 수 있으나 실제로는 **single-decision /
...
- **Network egress floor**: deny-by-default. egress 허용 도메인은 boot 에서
  로드된 _모든_ plugin manifest 의 `networkAccess.allowedDomains` UNION 으로
  계산되어 ASRT SHARED config 에 `strictAllowlist: true` 와 함께 설정된다
  (loopback proxy strict-union 바닥). manifest 에 도메인을 선언하지 않은
  plugin 의 sandboxed 자식은 네트워크 egress 가 없다. 단 이는 _per-worker_
...
```json
{
  ...
  "tools": [
    {
      "name": "meeting_read", "description": "회의 세션과 전사 상태를 읽는다.",
  "inputSchema": {
    "type": "object",
    "required": ["operation"],
    "properties": {
          "operation": { "enum": ["sessions",
  ...
  "sessionId": { "type": "string" }
    ...
    { "name": "meeting_write",   "description": "명시적으로 선택한 회의 녹음 작업을 수행한다.",
      "inputSchema": {
        "type": "object",
  ...
  "eventSubscriptions": ["calendar.event.started"],
      "notificationEvents": [
    {
      ...
      "titleField": "title",
        ...
        "skills": [{
          "id": "attendance", "path": "skills/attendance" }],
    "hooks": [{
      "id": "audit",
      "path": "hooks/audit.json" }],
      "mcpServers": [{
        "id": "meeting-local",
        "path": "mcp/meeting-local.json" }],
...
| `onboarding` | `PluginOnboardingSpec` (선택) | 첫 실행 안내 선언. `firstTask` 는 투어 직후 제안할 한 건, `highlights[]` 는 기능 소개·신규 기능 홍보를 최대 5건이며 각 항목의 `id` 는 플러그인 안에서 유일해야 한다. 모두 inert metadata 다. 호스트가 타일 오버레이 카드로 한 번씩 물어보고 사용자의 답(승인 / 다음에 다시 보기 / 그만 보기)을 `~/.lvis/onboarding/proposals.json` 에 `<pluginId>:<proposalId>` 키로 기록한다. 승인했을 때 벌어지는 일은 `action` 이 정하며 (`composer` 프리필 / `settings` 이동 / 없음), 턴을 시작하거나 도구를 호출하지 않는다. `settings` 의 `path` 는 `<탭>` 또는 `<탭>/<섹션>` 형식이라 옵션을 켤 수 있는 위치까지 이동하며, 호스트가 로드 시점에 두 부분 모두 이 빌드가 실제로 제공하는 탭·섹션인지 검증하고 아니면 매니페스트를 거부한다. |
...
**Tool `inputSchema`가 필요한 이유 — LLM 파라미터 추론 관찰**
...
generic schema(`{ payload: object }`)는 LLM에 파라미터 구조를 전달하지
못한다. 따라서 모든 manifest Tool 객체가 자신의 `inputSchema`를 직접
보유하며, Host와 SDK는 별도 schema map이나 추론 fallback을 두지 않는다.
...
- `onDisable(pluginId)` — 플러그인의 tear-down 직후 발화한다. 호스트는 이 콜백으로 `toolRegistry.unregisterByPlugin` / `conversationLoop.onPluginDisabled`를 호출해 transient host state를 정리한다. 발화 지점: `restartPlugin` stop phase, `restartAll` stop phase (per plugin), `disable`, `removePlugin`, `reloadPlugin` stop phase.
...
- `onActiveStateChange(pluginId, enabled)` — 사용자 활성/비활성 토글에서만 발화하며 runtime unload가 아니다. 비활성화 시 호스트는 ConversationLoop transient scope만 정리하고 ToolRegistry는 유지한다. 따라서 auth/config/UI 호출은 계속 runtime 도구를 실행할 수 있고, 모델 노출만 ConversationLoop tool scope에서 차단된다.
...
PR 3 에서 Microsoft Graph 인증이 호스트에서 플러그인으로 이전되었다. 이는 §9.4a 의 일반 정책 — _플러그인은 HostApi 만으로 호스트 자원 접근_ — 에 대한 정당한 예외이며, 이후 다른 OAuth-기반 플러그인 (Slack / Notion / Google 등) 도 동일 패턴을 따른다.
...
> `statusTool`/`loginTool`/`logoutTool` 은 visibility가 정확히 `["app"]`인
> pure `Tool` 객체를 가리킨다. 별도 app-action surface는 없다.
...
```jsonc
{
  "auth": {
    ...
    "statusTool": "msgraph_status",  // 필수, app-only Tool
    "loginTool": "msgraph_auth",     // 필수, app-only Tool
    "logoutTool": "msgraph_signout", // optional, app-only Tool
...
세 이름은 모두 `tools[]`의 app-only Tool을 가리켜야 한다. `manifest-validation.ts` 의 cross-field validator가 load-time에 이를 강제한다.
...
호스트는 defensive parse — 추가 필드는 무시. `outputSchema` 강제 검증은
Tool 계약 전반의 별 작업으로 deferred.
...
- `auth.statusTool` 의 `outputSchema` 강제 검증 (Tool outputSchema 인프라 큰 작업)
...
> `tools[]` 는 lower snake_case LLM tool name이며 (`msgraph_calendar_today`, `msgraph_email_start_watcher` 등), boot/runtime registration 경로에서도 동일한 이름이 그대로 전달된다. dotted form은 이벤트 이름에만 사용한다.
...
**결정**: Work Board를 **server-backed plugin POC** (`lvis-agent-hub` 서버 + `lvis-plugin-agent-hub` bridge) 에서 **호스트 1급 도메인 (host-integrated agentic work orchestrator)** 으로 전환한다. 이는 아래 §10.1 Roadmap 이 명시한 미구현 갭 — _"승인 후 실제 행위 실행은 아직 연결하지 않는다"_ — 을 실현하는 작업이다.
...
| 시나리오       | Input Classification | Notes | Local Index | Server Index |   the LLM backend    | Agent Hub |    Plugin     | Overlay Trigger |   Agent Approval    |
| ------------ | :-------: | :---: | :---------: | :----------: | :-------------: | :-------: | :-----------: | :-------------: | :-------------------: |
...
flowchart TB
    ...
    subgraph "Layer 2 — Client Intelligence"
        KW["① Input Classifier<br/>일반 입력 판별"]
...
| Slash command parsing            | Input Classifier      | Host 소유 command만 분류          |
| Skill invocation resolution      | Skill + Tool catalog  | bundled Skill 지침 + bounded tool_search |
...
**No Fallback Code 정합**: 부모-모델 fallback 은 _design-intent 안전 경로_(legacy alias 아님)이며, 발생 시 항상 `log.warn` 으로 audit 에 남긴다 — fallback 이 조용히 버그를 가리지 못하도록 강제. 비기술 사용자(총무/회계/인사/기획)가 프로필 `.md` 의 모델 ID 를 오타내도 sub-agent 가 opaque provider 에러로 죽지 않고 부모 모델로 degrade 한다.
