...
Manifest 는 각 tool 의 `_meta["xyz.lvis/pathFields"]` 로 filesystem path 판정
...
- **Gate**: `osToolSandbox` feature flag (Settings → 권한) 또는
  ...
  C/D-series 샌드박스 QA 가 green 될 때까지 Settings 토글로 opt-in. *Convergence
  plan*: C/D QA 통과 시 `src/data/settings-store.ts` 의
...
- **Read-relaxation ↔ plugin effect-boundary FILESYSTEM-CONTAINMENT 커플링
  ...
  control channel 을 유지하되 ASRT 0.0.64 의 공개
...
- **Plugin egress SOT**: `local-indexer` 의 long-lived Python worker provider
  egress 는 broker + `hostApi.hostFetch` chokepoint 로 수렴한다. `lge-api` 는
  Python worker 가 없고 Playwright 브라우저 SSO + 사내망 HTTP/HTTPS endpoint 를
  직접 다루는 예외이므로, 허용 호스트는 `lvis-plugin-lge-api/plugin.json` 의
...
**Migration map:** v4 의 `dangerous` 단일 카테고리는 v5 에서 폐지됨. `bash` → `shell`, `agent_spawn` / `ask_user_question` → `meta`. Plugin 의 filesystem path 판정 metadata SOT 는 tool 객체의 `_meta["xyz.lvis/pathFields"]` 이다 (#885 Phase R). Per-tool category 는 manifest 필드가 아니며 effective category 는 host-side `inspectHostRisk`/executor policy 가 invocation 별로 산출한다.
...
- **Plugin egress SOT**: `lvis-plugin-local-indexer` 의 Python worker provider
  ...
  (Tier A NetworkGuard) 로 호출한다. `lvis-plugin-lge-api` 는 Python worker 가
  없고 Playwright 브라우저 SSO + 사내망 HTTP/HTTPS endpoint 를 직접 다루는
  예외이므로, `lvis-plugin-lge-api/plugin.json` 의 `networkAccess.allowedDomains`
...
> **⚠️ 매니페스트 포맷 변경 (removed in #885 Phase R):** 아래 §9.2 전체 — manifest 예시,
> `toolSchemas` / `uiActions` map, auth-contract 하위 절 — 는 **pre-v6 (legacy)** 포맷이다.
> #885 Phase R 이후 tool 계약은 pure MCP `Tool[]` 하나로 통합됐다: 각 tool 은
> `{ name, description, inputSchema, _meta? }` 객체이고, renderer 노출 여부는
> `_meta.ui.visibility` (`["model"]` / `["app"]` / `["model","app"]`), path 판정은
> `_meta["xyz.lvis/pathFields"]`, per-tool category 는 manifest 필드가 아니라 host 분류다
> (`toolSchemas` / `uiActions` / per-tool `category` 는 삭제). 현행 계약은
> `docs/architecture/plugin-contract-v6-design.md` 참조. 아래는 historical 로 읽을 것.

매니페스트 스키마 (아래 §9.2 예시는 pre-v6)는 `@lvis/plugin-sdk/schemas/plugin-manifest.schema.json` (AJV strict, `additionalProperties: false`) 이 단일 진실 소스다. 호스트는 SDK 패키지의 스키마를 런타임에 resolve 하며 app-local schema extension, tool-name inference, compatibility grace 를 두지 않는다. 검증 플로우·에러 포맷·capability taxonomy·uiActions 보안 경계 등 상세 규격은 `docs/references/plugin-tool-schema-design.md` (v4) 에 정의되어 있으며, 아래는 호스트-측 구조와 필드 관계 요약이다.
...
```json
{
  ...
  "tools": ["meeting_start", "meeting_stop", "meeting_transcript", "meeting_sessions"],
  "uiActions": {
    "meeting_start": {},
    "meeting_push_chunk": {},
    "meeting_stop": {},
...
| `toolSchemas` | **`Record<string, { description, category?, pathFields?, workerId?, inputSchema, $schema? }>` (map form)** | LLM 파라미터 추론용 JSON Schema draft-07 + host policy metadata. `description` minLength 10, `category ∈ read/write/shell/network` 는 선택적 provider metadata 이며 effective category 는 host-side `inspectHostRisk`/executor policy 가 invocation 별로 산출한다. `pathFields[]` 는 filesystem path 판정 metadata 의 SOT 이고 dotted selector 를 허용한다. `workerId` 는 host-routed worker confinement 힌트이며 HostApi `spawnWorker` 경로로 검증된 worker 에만 효력이 있다. `inputSchema.type` 은 const `"object"` 를 요구한다. 런타임 payload 재검증은 수행하지 않는다. |
| `uiActions` | `Record<string, { description?: string }>` | Renderer `lvis:plugins:call` IPC 허용 allowlist. `tools[]` 와 별도 surface이며 UI 전용 runtime method 를 포함할 수 있다. 호출 시 `PluginRuntime.callFromUi()` 가 매번 `manifest.uiActions` 를 재확인한다. `tools[]` 에도 있는 action 은 ToolExecutor 권한 경로를 타며, UI-only action 은 auth status polling 을 제외하고 preload 가 확인한 active user activation 이 있어야 handler 로 전달된다. 플러그인은 별도 IPC 채널이나 직접 handler 호출 경로를 선언할 수 없다. |
...
> **v6 note (removed in #885 Phase R):** 아래 auth 계약은 유효하되 표현이 바뀌었다 —
> `statusTool`/`loginTool`/`logoutTool` 은 이제 `uiActions` map 이 아니라 visibility 가
> **정확히 `["app"]`** 인 pure `Tool` 객체를 가리킨다 (app-only; model 노출 금지). load-time
> validator 는 "auth tool 은 `tools[]` 밖 + `uiActions` 안" 대신 "auth tool 의
> `_meta.ui.visibility === ["app"]`" 를 강제한다. 아래 `uiActions` 표기는 pre-v6 로 읽을 것.
...
```jsonc
{
  "auth": {
    ...
    "statusTool": "msgraph_status",  // 필수, uiActions 안
    "loginTool": "msgraph_auth",     // 필수, uiActions 안
    "logoutTool": "msgraph_signout", // optional, uiActions 안 (있으면)
...
세 tool 이름은 **모두 `uiActions` 에 선언되어야 하며 `tools[]` 에 있으면 안 된다**. `manifest-validation.ts` 의 hand-rolled cross-field validator 가 load-time 에 이를 강제한다 (AJV 단독으로는 cross-object membership 표현 불가).
