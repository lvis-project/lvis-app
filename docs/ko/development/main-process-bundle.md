...
`import("./boot.js")`와 corporate CA 준비를 병렬로 시작한다. Boot 실패는 즉시
관찰하고, CA 준비가 끝나 splash 상태를 갱신한 뒤 boot 결과를 기다린다. 이는
loading 순서만 바꾸며 `bootstrap()` 내부 service 구성 순서는 바꾸지 않는다.
