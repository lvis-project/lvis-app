...
| Q3 | Auth | **MVP: API key per publisher (Bearer)** / Phase 3: SSO (portal.example.com OIDC) passthrough | API key 로 단순하게 시작, SSO 로 성숙도 올리기 |
...
- `https://marketplace.lvis.internal.portal.example.com` → `127.0.0.1:8080`
...
```json
{
  "baseUrl": "https://marketplace.lvis.internal.portal.example.com",
  "apiKey": null,
  "timeoutMs": 30000
}
```
...
lvis-marketplace config set baseUrl https://marketplace.lvis.internal.portal.example.com
...
2. **Server up**: IT 가 VM + nginx + corporate CA 인증서 준비, systemd 서비스 기동
...
| **M5** — Deploy to IT VM | systemd + nginx + corporate CA cert + 초기 catalog seed | 1d | **IT 협조 (VM 프로비저닝)** |
...
2. **사내 도메인 등록** — `marketplace.lvis.internal.portal.example.com` (또는 유사) 할당 + DNS record + corporate CA 서명 cert 발급 가능 여부
3. **nginx / systemd 권한** — LVIS 팀이 직접 운영 가능한지, IT 운영팀 관리 대상인지
4. **SSO 통합 일정** — portal.example.com OIDC client 등록 절차, 개발자가 직접 가능한지, IT 승인 필요한지
...
- **Multi-tenant** — organization 구분. MVP 는 single tenant (corporate 전체).
