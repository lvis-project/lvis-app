...
- **parent→child mid-run directive**: the fourth edge, added after the three above were live. `agent_guide` (root-only tool; sub-agents are blocklisted from it and its execute() refuses depth ≥ 1) → `SubAgentRunner.queueParentMessageToChild`. Authorization is the host-written spawn record (`isPersistedSpawnOfOrigin`), one hop and downward only — a sub-agent sender is refused before ownership is consulted, and there is no `to: "parent"` here because that direction is `agent_send`. A RUNNING child receives the directive on its own `queueGuidance` seam at the next round boundary; a SUSPENDED child gets a durable entry in `parent-directives.json` (same `subagent-messaging` namespace), delivered as `initialGuidance` on the next resume and acknowledged only when that turn concludes. Delivery is mailbox-first like the child edges, so a turn that ends before a round boundary keeps the directive rather than dropping it with the queue. A child that is neither live nor resumable is REFUSED (`child-not-resumable`) instead of queued — the resume gate accepts only INPUT_REQUIRED, so queuing there would store a message no path could deliver. The parent's words travel inside a `<lvis-parent-directive>` fence under a host-composed label, and a per-child pending cap bounds a flood.
...
- **Transport/auth floor:** every admitted interface is HTTPS, uses the A2A
  `JSONRPC` (JSON-RPC) binding and a supported protocol version, and declares
  at least one
...
- Agent Hub returns only the credential binding/revision ID plus bounded
  ...
  I/O.
- The first slice accepts A2A v1.0 over public HTTPS, the exact `JSONRPC`
  (JSON-RPC) binding, and HTTP Bearer
