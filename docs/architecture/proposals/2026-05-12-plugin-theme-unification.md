...
| `lvis-plugin-lge-api` | vanilla JS + detached BrowserWindow | local helper `lge-control.js:765-778 bootstrapHostTheme` — `getTheme()` + `onEvent` |
...
5. **React / vanilla-JS entry 분기** — `useTheme`(React) 와 `applyThemeFromHostEvent`(vanilla) 둘 다 `document.documentElement` 만 target. detached BrowserWindow (meeting recorder, lge-api detached) 와 scoped sidebar (local-indexer) 는 SDK API 가 못 가리켜서 각 플러그인이 자체 helper 작성.
...
// vanilla 플러그인 + detached window (lge-api / meeting recorder)
export function mount(host: PluginHost): PluginInstance {
...
| # | 레포 | 변경 | 트랙 | 비고 |
|---|---|---|---|---|
| 1 | `lvis-app` | `plugin-ui-shell` prime 을 detached-window / scoped sidebar parity 까지 확장 | P0 | SDK 변경 없음. meeting recorder / lge-api detached 측 host 측 paint 가 먼저 통일. |
...
| 7 | `lvis-plugin-lge-api` | `bootstrapHostTheme` 삭제 → `primeTheme(bridge, { target: detachedDoc })` 한 줄 | — | PR-1 (host detached-window parity) 머지 후. |
...
- `lvis-plugin-lge-api/src/ui/lge-control.js:765-778` — detached window pattern
