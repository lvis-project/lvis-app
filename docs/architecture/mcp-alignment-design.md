...
marketplace repos; installed here: `ep-api`, `local-indexer`), so the pilot is an
...
`ep-api`) ALREADY fail AJV manifest validation at load and register nothing today
