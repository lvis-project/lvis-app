...
1. All 6 first-party plugins + template publish colocated manifests (a3):
   `for r in meeting ms-graph work-assistant local-indexer ep-api git template; do node -e "process.exit(typeof require('../lvis-plugin-'+process.argv[1]+'/plugin.json').tools[0]==='object'?0:1)" "$r" || echo "NOT MIGRATED: $r"; done`
...
`lvis-app` (host) · `lvis-plugin-sdk` (v5.22.0 → **v6**) · 6 plugins (`meeting`, `ms-graph`, `work-assistant`, `local-indexer`, `ep-api`, `git`) · `lvis-plugin-template` · `lvis-marketplace` (catalog `PluginMarketplaceItem.tools`/`.toolSchemas` mirror + publisher schema).
