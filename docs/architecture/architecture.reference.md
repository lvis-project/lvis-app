...
graph TB
    ...
    subgraph "🏢 Enterprise Infrastructure"
        subgraph "Core Services"
            corporateNIE["Internal search<br/>(사내 LLM System)"]
    ...
    CLIENT <-->|"WebSocket/gRPC"| corporateNIE
...
block-beta
    ...
    block:LAYER3["Layer 3: Communication — Protocol Layer"]
        C1["Internal search<br/>Session"] C2["Agent Hub<br/>MessageBus"] C3["Marketplace<br/>API Client"] C4["Sync<br/>Engine"]
    end
    block:LAYER4["Layer 4: Server — Enterprise Services"]
        D1["Internal search<br/>LLM"] D2["Agent Hub<br/>Board"] D3["Marketplace<br/>Hub"] D4["Server Index<br/>& Knowledge"]
    end
...
graph LR
    ...
    subgraph PILLAR4["🧠 Internal search"]
        direction TB
        P4A["LLM Inference"]
        P4B["Model Router"]
        P4C["Context Manager"]
...
graph TB
    ...
    subgraph "Layer 3 — Communication"
        direction LR
        L3_SESSION["Internal search Session Manager"]
    ...
    subgraph "Layer 4 — Enterprise Server"
        direction LR
        L4_LLM["Internal search LLM System"]
...
sequenceDiagram
    ...
    participant Internal search as Internal search
    ...
    and
        Config->>Internal search: 3c. Session handshake
        Internal search-->>App: Session ID + model config
    end
...
flowchart TB
    ...
    DIRECT_LLM["Internal search 직접 대화"]
    ...
    RESULT["Tool Result"]
    LLM_CALL["Internal search 추론 요청"]
    RESPONSE["응답 생성"]
...
| 4 | 의도 기반 | "이 문서 요약해줘" | Intent → Route Engine → Internal search |
...
flowchart TB
    ...
    RESOLVER -->|"LLM 대화"| corporateNIE_SESSION["Internal search Session<br/>(직접 추론)"]
...
5. Internal search Fallback       → 위 모두 해당 없으면 LLM 직접 대화
```
...
stateDiagram-v2
    state "기본 클라이언트" as BASE {
        [*] --> Chat: 부팅
        Chat --> Internal search: 메시지 전송
        Internal search --> Chat: 응답
...
sequenceDiagram
    ...
    alt 이영희 온라인
        ...
        AgentB->>Internal search: Context 조회 (권한 확인)
        Internal search-->>AgentB: 권한 OK + 파일 위치
...
flowchart TB
    ...
    subgraph "Server"
        corporateNIE_CALL["⑧ Internal search 추론<br/>요약/정리/포맷팅"]
    ...
    MID_SUM -->|"LLM 요약"| corporateNIE_CALL
    FINAL -->|"LLM 포맷팅"| corporateNIE_CALL
    FINAL --> AGENT_SHARE --> SRV_STORE_SAVE

    corporateNIE_CALL -->|"요약 결과"| MID_SUM
    corporateNIE_CALL -->|"최종 문서"| FINAL
```
...
graph TB
    ...
    subgraph "On-Premise Datacenter"
        subgraph "Internal search Cluster"
            ...
            LLM_1["Internal search Node 1"]
            LLM_2["Internal search Node 2"]
            LLM_N["Internal search Node N"]
            LLM_LB --> LLM_1
            LLM_LB --> LLM_2
            LLM_LB --> LLM_N
        end
...
| **Server** | Internal search | 사내 LLM 시스템 (독자 인프라) |
...
| Multi-provider `ApiClient` | Internal search Session — 사내 LLM 엔드포인트 추상화 |
