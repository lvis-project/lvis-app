...
- **다중 emit 허용 (progressive enrichment)** — payload 자체가 advisory (`{authenticated: boolean}` 정도) 이고 host 는 매번 statusTool 을 재호출하는 pull-model 이므로, 한 로그인 시퀀스에서 동일 `${id}.auth.changed{true}` 이벤트가 여러 번 emit 되는 것이 정상이다. 전형적인 패턴: 플러그인이 (a) 가벼운 토큰/쿠키 확인 직후 1차 emit (UI auth-overlay/spinner 즉시 hide), (b) 외부 디렉토리 / SSO lookup 으로 사용자 식별자(예: 사번/이름) 를 보강한 뒤 2차 emit (사이드바 badge 가 풍부한 identity 로 refresh). host hook 은 매 emit 마다 statusTool 재호출이 idempotent 하다고 가정 — flicker 가 보이면 그건 hook 측 다이프 부재 회귀이지 plugin 의 emit 횟수 문제가 아니다. `lastEmittedAuthState` 같은 plugin-측 dedup 가드는 *transition* (`true → false`, `false → true`) 에만 의미가 있고, progressive identity refresh 같이 같은 boolean 으로 다시 emit 하는 케이스는 의도적으로 우회한다. 구체 plugin 의 emit 점들 (예: `lge-api` 의 cookie-harvest + Tier-1 LDAP 보강 2-stage) 은 `docs/references/plugin-tool-schema-design.md` 의 auth 절에서 plugin-specific 예시로 다룬다.
