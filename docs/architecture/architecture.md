...
flowchart TB
    ...
    DIRECT_LLM["Internal search 직접 세션<br/>(일반 대화)"]
    ...
    LLM_CALL["Internal search 추론<br/>(사내 LLM)"]
...
| LLM 의존성 | 임베딩: OpenAI `text-embedding-3-small` | Phase 1: OpenAI API key 필요. Phase 2: BAAI/bge-m3 로컬 임베딩 또는 Internal search 후속 |
| 온라인 전제 | OpenAI 임베딩 API 필요 (인덱싱 시) | Phase 1: OpenAI key로 동작. Phase 2: Internal search 로컬 임베딩으로 오프라인화 예정 |
...
| **Hybrid Ranker** | TypeScript `HybridRetriever`가 worker `/search/bm25` + `/search/vector` + `MockCloudIndexAdapter` 결과를 RRF(k=60, weights {bm25:0.5, vec:0.5, cloud:0.0}) 결합. Phase 2에서 Internal search/사내 ES Cloud 실연결 시 weights 재정규화 |
...
| **Out-of-Scope** (의도적 후속) | LightRAG knowledge graph (Phase 1.5 feature flag, 동시성 race condition 해결 후), BAAI/bge-m3 로컬 임베딩 (Phase 2), Internal search LLM (Phase 2), 사내 ES/Milvus 실연결 (후속 라운드), PPTX/XLSX OCR (Phase 2 Vision) |
...
flowchart TB
    ...
    subgraph "③ Internal search 추론"
        STREAM_API["Internal search API 스트리밍 호출<br/>(SSE — Server-Sent Events)"]
...
| **5. Internal search 스트리밍 호출** | `Internal searchSession.stream()` | SSE 기반 스트리밍 요청. 토큰이 도착할 때마다 UI에 증분 렌더링 | 온라인 전제 |
...
| **9. 도구 실행** | `StreamingToolExecutor.run()` | 병렬 실행 가능. 각 도구 결과를 `tool_result` 메시지로 조립 후 Internal search에 재전송 | Agentic Loop |
...
Internal search 응답은 SSE(Server-Sent Events)로 전달되며, 사용자는 전체 응답 완료를 기다리지 않고 토큰 단위로 즉시 확인한다.

```mermaid
sequenceDiagram
    participant UI as Chat UI<br/>(Electron Renderer)
    participant MAIN as Main Process<br/>(IPC Bridge)
    participant SESSION as Internal searchSession<br/>(Rust NAPI)
    participant LLM as Internal search API<br/>(사내 LLM)
...
| **병렬 도구 실행** | Internal search가 복수 tool_use를 동시 요청하면 `StreamingToolExecutor`가 병렬 실행 후 결과를 한 번에 반환 |
| **Agentic Loop** | tool_use → tool_result → 재추론 사이클이 자동 반복. Internal search가 tool_use 없는 응답을 생성하면 루프 종료 |
| **인터럽트 처리** | 사용자가 도구 실행 중 취소(Ctrl+C / UI 버튼) 시 `StreamingToolExecutor.abort()` 호출. 대화 히스토리는 보존되어 이어서 대화 가능 |
| **오류 격리** | 개별 도구 실행 실패 시 해당 tool_result에 에러 메시지를 담아 Internal search에 전달. Internal search가 재시도 또는 대안 제시 |

#### 4.5.4 컨텍스트 관리 — Auto-Compact

대화가 길어지면 히스토리 토큰이 Internal search 컨텍스트 윈도우를 초과한다. Auto-Compact가 이를 자동 관리한다.
...
flowchart LR
    ...
    SUMMARY["Internal search 자체 요약<br/>(이전 대화 압축)"]
...
| 압축 방식 | Internal search가 compact boundary 이전 메시지를 자체 요약 |
| 공간 회수 | 약 40~60% |
...
| Hook | 실행 조건 | 동작 |
| --- | --- | --- |
| **Auto-Compact** | 토큰 사용률 ≥ 80% | 이전 대화 Internal search 자체 요약 → 공간 확보 |
...
```mermaid
sequenceDiagram
    participant LLM as Internal search
...
| **세션 메타데이터** | 제목(자동 생성), 생성일시, 마지막 활동, 토큰 사용량 | Internal search로 제목 자동 생성 |
...
| Stream to Claude API | `Internal searchSession.stream()` (SSE) | Internal search 사내 LLM 엔드포인트 |
...
| Auto-compact | `AutoCompact.check()` | 동일 패턴 — Internal search 자체 요약 방식 |
...
flowchart TB
    subgraph "Layer 1 — Tool Registry Filter (컨텍스트 빌드 전)"
        ...
        VISIBLE["Internal search에 전달되는<br/>도구 스키마 목록"]
    end
    ...
    DENY_RULES --> FILTER
    FILTER -->|"차단 도구 제거<br/>💡 Internal search는 차단된 도구의<br/>존재 자체를 알 수 없음"| VISIBLE
...
| Layer | 시점 | 역할 | 보안 효과 |
| --- | --- | --- | --- |
| **L1 — Registry Filter** | System Prompt 빌드 전 | Governance deny 규칙으로 차단 도구를 LLM 컨텍스트에서 **제거** | Internal search가 존재를 모르므로 할루시네이션 호출 원천 차단 |
...
```json
{
    "id": "com.ep.meeting-recorder",
    "name": "회의록 녹음",
    "version": "1.2.0",
    "description": "STT 기반 회의록 자동 작성 플러그인",
    "author": "DX Platform Team",
    "permissions": ["microphone", "local-storage", "internal-search-session", "ui-slot:sidebar", "ui-slot:toolbar"],
...
```json
{
  "mcpServers": {
    ...
    "hr-system": {
      "url": "https://hr-api.portal.example.com/mcp",
      "transport": "sse",
      "auth": "sso"
    },
    "erp-realtime": {
      "url": "wss://erp.portal.example.com/mcp",
      "transport": "websocket",
      "auth": "sso"
    }
  }
...
| **서명 검증** | Corporate Internal Root CA **필수** (실패 시 load 거부) | 정책에 따라 (warn / require / off) |
    ...
    "managedPlugins": [
      {
        "id": "lvis-plugin-pageindex",
        "version": "0.2.0",
        "source": "https://internal.portal.example.com/lvis/marketplace/api/v1/plugins/lvis-plugin-pageindex/0.2.0",
        "sha256": "...",
        "forceInstall": true,
        "autoUpdate": true
      }
...
| Auto-compact                     | AutoCompact           | 동일 패턴 — Internal search 자체 요약 |
