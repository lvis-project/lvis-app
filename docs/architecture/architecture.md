...
**AAD Seamless SSO — 호스트 레벨 Negotiate enabler (PR #699)**: corp-joined Windows PC 가 사내망에 있을 때 ms-graph 의 MSAL popup 을 비밀번호 입력 없이 silent 통과시키기 위해, `src/main.ts` 가 `app.whenReady()` 이전에 Chromium 의 `auth-server-allowlist` 스위치를 두 Microsoft AAD apex 호스트 (`login.microsoftonline.com`, `autologon.microsoftazuread-sso.com`) 로 설정한다. 이는 §17 C1 corp-CA loader 와 동일 결의 *호스트 레벨 네트워크 enabler* — plugin-specific 코드가 아니라 OS Kerberos ticket ↔ Chromium HTTP-Auth 협상 layer 의 generic 정합 (어떤 AAD 기반 plugin 도 자동으로 혜택). MSAL 의 OAuth 상태 머신은 그대로 plugin 소유 — 본 enabler 는 그 아래 HTTP layer 의 Negotiate 챌린지를 OS ticket 으로 응답할지 여부만 결정. `--auth-negotiate-delegate-allowlist` 는 의도적으로 비워 ticket forwarding 차단. `LVIS_DISABLE_AAD_SSO=1` 환경 변수 opt-out 제공. wildcard glob 금지 (`evil-login.microsoftonline.com` 매칭) — `src/__tests__/main-aad-seamless-sso.test.ts` 가 두 apex 정확 일치 + delegate 부재 + env-gate 존재를 lock-in.
