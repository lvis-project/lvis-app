...
| `uiActions` | `Record<string, { description?: string }>` | Renderer `lvis:plugins:call` IPC 허용 allowlist. `tools[]` 와 별도 surface이며 UI 전용 runtime method 를 포함할 수 있다. 호출 시 `PluginRuntime.callFromUi()` 가 매번 `manifest.uiActions` 를 재확인한 뒤 ToolExecutor 로 위임한다. 플러그인은 별도 IPC 채널이나 직접 handler 호출 경로를 선언할 수 없다. |
