...
- **Read-relaxation ↔ sandbox-active 커플링**: foreground plugin read-relaxation
  (`src/tools/executor.ts` executeOne) 은 `hostClassifiesRisk` ON **AND
  `sandboxActiveProvider()` (= `isAsrtSandboxActive`) true** 일 때만 발화한다.
  relaxation 은 pre-exec ask 를 제거하고 effect-boundary gate 에 의존하는데, 그
  boundary 는 OS 샌드박스가 ACTIVE 일 때만 off-hostApi mutation(direct node:fs /
  bare fetch / detached async frame)을 contain 한다. 따라서 sandbox 비활성
  (degrade/off) 호스트에서는 relaxation 이 미발화하고 known-safe pre-exec ask 가
  유지된다 → `hostClassifiesRisk`-ON 이 전 플랫폼에서 안전. 두 production 생성
  지점(`boot.ts` pluginSurfaceExecutor + `engine/conversation-loop.ts`)이
  `isAsrtSandboxActive` 를 주입한다.
