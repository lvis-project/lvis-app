# Archived: legacy Claude Code directives

> Superseded by the repository-root `AGENTS.md`. This file is retained only as
> historical design and incident context. Coding agents must not auto-load or
> treat any directive below as normative; follow the current SoT documents
> linked from `AGENTS.md` instead.
