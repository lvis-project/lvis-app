...
- **`osToolSandbox` now defaults ON on `win32`** (joining `darwin`), justified by the install-time provisioning above (`src/data/settings-store.ts`). `linux` stays opt-in until the C/D-series QA is green. If a host reaches first launch without a provisioned sandbox, win32-not-ready still does not hard-throw — the boot gate degrades gracefully and the runtime panel is the repair fallback.
...
- **⚠️ Cannot be verified in CI** — the install-time provisioning needs a real elevated Windows install run. The NSIS target currently ships `oneClick:true` + `perMachine:false` (a per-user, non-elevated installer), so `srt-win.exe` self-elevates and raises its own single UAC during install; a truly UAC-free install-time provision would require flipping to `perMachine:true` (a separate installer-posture decision, not made here).
