...
- **첫 실행 안내의 강조 표시가 대상 위에 그대로 놓입니다.** 안내가 가리키는 버튼이나
  입력창에 강조 테두리가 직접 붙고, 설명 카드도 그 요소에 붙어서 열립니다. 번역 문구가
  길어 카드가 커져도 카드가 대상을 벗어나거나 덮지 않고, 상단 알림 띠가 열리거나 작성창이
  커져 화면이 움직여도 강조와 카드가 대상을 따라갑니다.
