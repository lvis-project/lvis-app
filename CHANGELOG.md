...
- **첫 실행 안내가 가리키는 것이 실제로 보입니다.** 안내가 켜지면 화면이 어두워지되 지금
  가리키는 버튼이나 입력창만 원래 밝기로 남고, 그 둘레에 강조 테두리가 그려집니다. 이전에는
  강조가 화면에 나타나지 않았습니다. 설명 카드도 그 요소에 붙어서 열리고, 번역 문구가 길어
  카드가 커지거나 상단 알림 띠가 열려 화면이 움직여도 강조와 카드가 대상을 따라갑니다. 밝게
  남은 그 자리를 누르면 안내가 닫히지 않고 그 입력창으로 포커스가 옮겨가므로, “여기에 입력해
  보세요” 단계를 안내를 잃지 않고 따라갈 수 있습니다. 안내가 설명 중인 버튼이 눌리지는
  않으며, 바깥을 누르면 종전대로 안내가 닫힙니다.

- **첫 실행 안내의 마지막 단계가 지금 쓰는 모델을 가리킵니다.** 예전 하단 상태바 자리를
  가리키고 있어 그 단계에서는 아무것도 강조되지 않았습니다. 이제 입력창 아래 상태 줄의 모델
  이름을 가리키고, 문구도 거기서 모델을 바꿀 수 있다는 설명으로 바뀌었습니다.

- **Tailnet 접속이 모델 프로바이더와 같은 접힘 카드가 됐습니다.** 원격 연결 탭의 Tailnet 항목은
  Tailscale이 준비된 데스크톱에서 “연결 준비됨” 상태와 로그인 계정·이 컴퓨터의 테일넷 이름을
  한 줄로 보여주고, [테일넷 연결] 버튼 하나를 제시합니다. 그 버튼을 누르면 포트·접근 경계·공유·웹
  주소를 앱이 정하고 Tailscale Serve까지 한 번에 켭니다. 포트 번호처럼 대신 정할 수 있는 값은
  더 이상 묻지 않으며, 기본 포트가 이미 사용 중이면 비어 있는 포트를 골라 저장해 다음 실행에도
  같은 주소를 유지합니다. 버튼을 누르면 먼저 이 데스크톱의 Tailscale 상태를 다시 읽고, 준비돼
  있으면 그대로 연결까지 진행하며, 그동안 버튼 자체가 회전 표시와 함께 “연결하는 중…” 으로
  바뀝니다. 따로 확인 버튼을 누를 필요가 없습니다. Tailscale이 아직 준비되지 않았다면 무엇이
  부족한지 알려주는 안내 문장이 뜨고, 같은 버튼을 다시 누르면 다시 확인합니다. 설정을 마치면 주소·포트·Serve 상태를 한 줄씩 보여주는 카드가 남고,
  항목마다 직접 정하고 싶을 때만 “수동으로 설정” 으로 기존 상세 양식을 펼칩니다. 모든 화면은
  설정 화면 안에서 그려지고 별도의 창을 띄우지 않습니다.

- **설정의 “원격 표면” 을 “원격 연결” 로 바꾸고, 같은 계열의 직역 문구를 다듬었습니다.**
  “웹 표면”, “루프백 HTTP 표면”, “외부 표면 입력” 처럼 뜻이 바로 오지 않던 표현을 각각
  “웹으로 접속”, “루프백 HTTP 서버”, “이 기기 밖에서 들어온 입력” 로 고쳤습니다.

- **서브에이전트 탭은 지금 시작한 서브에이전트에만 열립니다.** 예전에 서브에이전트를 돌렸던
  대화를 열 때 작업 패널이 저절로 펼쳐지지 않고, 그 대화의 서브에이전트 목록은 작업 패널의
  런처에서 직접 열 수 있습니다. 서브에이전트 탭은 그 대화의 것이라 다른 대화로 옮기면 함께
  닫히고, 서브에이전트를 쓴 적 없는 대화의 작업 패널에 빈 서브에이전트 탭이 붙어 나오지
  않습니다.

- **상단 알림 띠가 부드럽게 나타나고 사라집니다.** 플러그인 로그인 취소 같은 알림이 창 위쪽에
  뜨고 사라질 때 사라지는 순간까지 그려진 채로 접히며, 아래 내용이 함께 움직입니다. 시스템의
  동작 줄이기 설정을 켜면 즉시 바뀝니다.
...
- **바깥으로 나가는 요청이 이 컴퓨터의 네트워크 설정을 따릅니다.** 프록시를 거쳐야 하는
  망에서는 모델 목록이 비어 있거나 불러오기가 실패했고, 플러그인 기능도 같은 자리에서
  멈췄으며, 화면에는 이유가 나오지 않았습니다. 앱 안에는 두 갈래의 통신 경로가 있었고,
  그중 하나가 이 컴퓨터의 프록시 설정과 인증서 저장소를 읽지 않은 채 곧장 나가고
  있었습니다. 이제 모델 목록, 마켓플레이스 상태 확인, 플러그인 배포 정책 문서, 텔레그램
  연결까지 앱이 바깥과 주고받는 모든 통신이 브라우저와 같은 경로를 씁니다. 회사 인증서를
  설정해 둔 경우에도 같은 경로를 따르므로, 사내망에서만 열리는 주소도 별도 설정 없이
  이어집니다.

- **파이썬 도구도 같은 프록시와 인증서를 씁니다.** 문서 색인 같은 기능이 파이썬 꾸러미를
  내려받을 때 프록시를 거치지 않아 사내망에서 설치가 멈췄습니다. 이제 앱이 이 컴퓨터의
  설정에서 프록시 주소를 읽어 파이썬 도구에 직접 넘기고, 회사 인증서를 설정해 두면 공개
  인증서와 함께 묶은 파일을 만들어 가리킵니다. 두 가지를 함께 담는 이유는 이 방식이 기존
  인증서 목록에 더하는 것이 아니라 통째로 대신하기 때문으로, 회사 인증서만 담으면 공개
  주소에서 내려받는 것이 모두 실패합니다.

- **응답하지 않는 서버에 플러그인이 매달려 있지 않습니다.** 연결은 받아 놓고 답하지 않는
  서버를 만나면 플러그인 요청이 끝나지 않았고, 화면에서는 그저 느린 것처럼 보였습니다.
  이제 각 요청에 응답 시작까지의 제한 시간을 둡니다. 내려받기처럼 본문이 큰 응답은
  시작된 뒤로는 끊기지 않으므로, 큰 파일을 받는 도중에 잘리지 않습니다.

- **모델을 불러오는 프로바이더가 "연결됨" 으로 보입니다.** 목록을 정상적으로 제공하는
  프로바이더인데도 원격 연결 목록에서 "주의 필요" 로 표시되는 경우가 있었습니다. 이제
  실제로 쓸 모델이 하나도 없을 때만 주의로 표시합니다.

- **바깥으로 나가면 안 되는 주소를 한 기준으로 막습니다.** 앱 안에 주소를 판단하는 곳이
  두 군데 있었고 서로 기준이 달라, 한쪽이 막는 주소를 다른 쪽은 통과시켰습니다. 이제 한
  기준으로 합쳤고, 문서용·시험용으로 예약된 주소나 멀티캐스트처럼 실제로 도달할 수 없는
  주소도 함께 막습니다. 내 네트워크 접근을 켜는 설정은 그대로 내 네트워크에만 적용되며,
  예약된 주소까지 열지 않습니다.

- **Tailnet 항목이 같은 제목을 두 번 보여주지 않습니다.** 원격 연결의 Tailnet 항목을
  펼치면 바로 아래에 사실상 같은 제목이 한 번 더 나왔습니다. 이제 안쪽 제목은 그 자리에서
  하는 일("이 데스크톱 설정")을 가리킵니다.
