...
- **답을 기다리는 대화는 사이드바에 노란 점으로 표시됩니다.** 다른 판에 가려지거나, 닫힌
  작업 패널·다른 탭 뒤에 있거나, 지금 어느 판에도 열려 있지 않은 대화가 승인이나 질문에
  멈춰 있으면 사이드바의 그 대화 줄(사이드 챗이면 부모 대화 줄에도), 가린 판의 최대화
  버튼, 작업 패널 토글과 사이드 챗 탭, 플러그인 메뉴에 같은 노란 점이 붙습니다. 화면에
  이미 보이는 카드에는 점이 붙지 않습니다.
