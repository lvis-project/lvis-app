...
- **모든 대화가 사이드바 대화 탭에 나타납니다.** 지금까지 대화 탭은 본 채팅 저장소만
  읽었기 때문에, 루틴이 실행한 대화와 워크스페이스 사이드 챗은 사이드바에서 볼 길이
  없었습니다. 이제 본 대화·루틴 실행·워크보드 실행이 각각 알맞은 아이콘과 함께 한
  줄씩 나타나고, 사이드 챗은 그 대화가 시작된 본 대화 아래에 들여쓰기로 붙습니다.
  누르면 루틴 실행은 대화로 열리고, 워크보드 실행은 작업 상세로, 사이드 챗은 그
  대화를 연 뒤 그 타일의 사이드 챗 탭으로 열립니다. 본 대화가 아닌 줄은 열기만 되며
  이름 바꾸기·보관·고정·끌어 옮기기는 되지 않습니다. 서브에이전트는 이미 그 대화
  안의 서브에이전트 탭에서 볼 수 있으므로 목록에 따로 나타나지 않습니다.

- **워크보드 작업의 실행 대화가 사이드바 대화 목록에 나타납니다.** 작업을 실행하면
  그 대화는 서브에이전트 저장소에 남아 사이드바에서 볼 길이 없었습니다. 이제 실행된
  작업마다 한 줄이 워크 아이콘과 작업 제목으로 대화 탭에 나타나며, 누르면 워크보드의
  작업 상세가 열리고 가장 최근 실행 기록이 펼쳐집니다. 이 줄은 이름 바꾸기·보관·
  고정·끌어 옮기기가 되지 않으며, 브리핑 실행은 나타나지 않습니다.
