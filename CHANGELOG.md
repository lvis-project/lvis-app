...
- **Tailnet 접속을 설정에서 단계별로 안내합니다.** 원격 연결 탭의 Tailnet 항목이 Tailscale
  상태 확인 → 설정 방식 → 적용 → 완료의 네 단계로 바뀌었고, 자동을 고르면 포트·접근 경계·공유·웹
  주소를 앱이 정하고 Tailscale Serve까지 한 번에 켭니다. 포트 번호처럼 대신 정할 수 있는 값은
  더 이상 묻지 않으며, 기본 포트가 이미 사용 중이면 비어 있는 포트를 골라 저장해 다음 실행에도
  같은 주소를 유지합니다. 설정을 마치면 주소·포트·Serve 상태를 한 줄씩 보여주는 카드가 남고,
  항목마다 직접 정하고 싶을 때만 “수동으로 설정” 으로 기존 상세 양식을 펼칩니다. 모든 단계는
