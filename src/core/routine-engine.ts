/**
 * Routine Engine — §7 Daily Briefing
 *
...
import {
  parseRoutineEvent,
  type RoutineEvent,
  type RoutineMailActionItem,
} from "./routine-events.js";
...
export interface DailyBriefingOptions {
   ...
   * RoutineTriggerCoordinator (e.g. "schedule:08:30", "meeting-in-10m",
...
export class RoutineEngine {
  ...
  getCalendarEvents(): CachedCalendarEvent[] {
    return [...this.calendarEventsCache];
  }

  updateMailActions(items: RoutineMailActionItem[]): void {
    this.mailActionsCache = items;
  }
  ...
  collectEvent(type: string, data?: unknown): void {
    const routineEvent = parseRoutineEvent(type, data);
    if (!routineEvent) return;
    this.applyRoutineEvent(routineEvent);
  }

  private applyRoutineEvent(event: RoutineEvent): void {
    if (event.type === "routine.snapshot.calendar") {
      this.updateCalendarEvents(event.events);
      return;
    }
    if (event.type === "routine.snapshot.calendar.invalidated") {
      this.updateCalendarEvents([]);
      return;
    }
    if (event.type === "routine.snapshot.mail") {
      this.updateMailActions(event.items);
      return;
    }
    if (event.type === "routine.snapshot.mail.invalidated") {
      this.updateMailActions([]);
      return;
    }

    if (event.type === "routine.item.created") {
      this.appendRoutineEvent(event);
      return;
    }
  ...
  collectBriefingItems(now: Date = new Date()): BriefingItem[] {
    ...
    this.collectRoutineItems(recentEvents, items);
  ...
  private collectRoutineItems(
    recentEvents: Array<{ type: string; data: unknown; timestamp: string }>,
    items: BriefingItem[],
  ): void {
    for (const event of recentEvents) {
      if (
        event.type === "routine.item.created" &&
        event.data &&
        typeof event.data === "object" &&
        "item" in (event.data as Record<string, unknown>)
      ) {
        items.push({ ...((event.data as { item: BriefingItem }).item) });
        continue;
      }

      if (
        (event.type === "routine.signal.upcoming" || event.type === "routine.signal.action-needed") &&
        event.data &&
        typeof event.data === "object" &&
        "category" in (event.data as Record<string, unknown>) &&
        "title" in (event.data as Record<string, unknown>)
      ) {
        const signal = event.data as {
        ...
        items.push({
          ...
          priority: signal.priority ?? (event.type === "routine.signal.action-needed" ? "high" : "medium"),
          title: signal.title,
  ...
  private async _generateDailyBriefingInner(options: DailyBriefingOptions): Promise<DailyBriefingResult> {
    ...
    //    "triggered" (Sprint 3-A-2 RoutineTriggerCoordinator non-idle signal).
    ...
    try {
      ...
      console.warn("[routine-engine] LLM briefing failed, falling back to text:", (err as Error).message);
  ...
  getBriefingPromptData(items?: BriefingItem[], now: Date = new Date()): string {
    ...
    const emailDetails = this.mailActionsCache.map((d) =>
        `  - "${d.subject}"${d.sender ? ` (from: ${d.sender})` : ""}${d.deadline ? ` [마감: ${d.deadline}]` : ""}${d.intent ? ` — ${d.intent}` : ""}`,
      );
  ...
  private appendRoutineEvent(event: RoutineEvent): void {
    this.eventLog.push({
      type: event.type,
      data: event,
...
export type ProactiveEngineDeps = RoutineEngineDeps;
export { RoutineEngine as ProactiveEngine };
