...
describe("SettingsService system — close behavior (PR #1032)", () => {
  ...
  it("defaults closeBehavior to 'hide-to-tray' on a fresh install", () => {
    const service = new SettingsService({ userDataPath });
    expect(service.get("system")).toEqual({ closeBehavior: "hide-to-tray", appMode: "work", localApiServer: false, sidePanelWidth: 448 });
  });
  ...
  it("round-trips a 'quit' preference across restart", async () => {
    ...
    expect(service.get("system")).toEqual({ closeBehavior: "quit", appMode: "work", localApiServer: false, sidePanelWidth: 448 });

    const reloaded = new SettingsService({ userDataPath });
    expect(reloaded.get("system")).toEqual({ closeBehavior: "quit", appMode: "work", localApiServer: false, sidePanelWidth: 448 });
  });
...
describe("SettingsService system — workspace appMode", () => {
  ...
  it("patching appMode does not clobber a prior closeBehavior", async () => {
    ...
    await service.patch({ system: { closeBehavior: "quit" } });
    await service.patch({ system: { appMode: "chat" } });
    expect(service.get("system")).toEqual({ closeBehavior: "quit", appMode: "chat", localApiServer: false, sidePanelWidth: 448 });
  });

  it("round-trips a localApiServer=true preference across restart", async () => {
    const service = new SettingsService({ userDataPath });
    await service.patch({ system: { localApiServer: true } });
    expect(service.get("system")).toEqual({ closeBehavior: "hide-to-tray", appMode: "work", localApiServer: true, sidePanelWidth: 448 });
