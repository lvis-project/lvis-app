...
describe("SettingsService remote A2A canonical route-control origin", () => {
  ...
  it("accepts the canonical root slash and rejects a value URL would rewrite", async () => {
    ...
    for (const noncanonical of [
      ...
      await service.patch({ a2aRemote: { routeControlBaseUrl: noncanonical } } as never);
...
describe("SettingsService marketplace defaults", () => {
  ...
  it("defaults to real-cloud against the local marketplace server", () => {
    ...
    expect(service.get("marketplace")).toEqual({
      ...
      updateCheckEnabled: DEFAULT_SETTINGS.marketplace.updateCheckEnabled,
      offlineCacheEnabled: DEFAULT_SETTINGS.marketplace.offlineCacheEnabled,
...
describe("SettingsService system — pinned project roots", () => {
  ...
  it("keeps only known sidebar groups, once each, and round-trips the folded set across restart", async () => {
    const service = new SettingsService({ userDataPath });
    await service.patch({
      system: { closeBehavior: "quit", appMode: "work", sidebarClosedGroups: ["plugins", "bogus", "plugins", "features"] as never },
    });
    expect(service.get("system").sidebarClosedGroups).toEqual(["plugins", "features"]);

    const reloaded = new SettingsService({ userDataPath });
    expect(reloaded.get("system").sidebarClosedGroups).toEqual(["plugins", "features"]);
  });

  it("ignores a non-list sidebarClosedGroups patch and keeps the stored set", async () => {
    const service = new SettingsService({ userDataPath });
    await service.patch({ system: { closeBehavior: "quit", appMode: "work", sidebarClosedGroups: ["features"] } });
    await service.patch({ system: { closeBehavior: "quit", appMode: "work", sidebarClosedGroups: "features" as never } });
    expect(service.get("system").sidebarClosedGroups).toEqual(["features"]);
  });
});
...
describe("SettingsService system — workspace appMode", () => {
  ...
  it("ignores an invalid sidePanelWidth patch and preserves prior value", async () => {
    ...
    expect(service.get("system").sidePanelWidth).toBe(700);
  });
  ...
  it("round-trips per-tab vertical split percents (rounded) across restart", async () => {
    ...
    await service.patch({
      system: {
        ...
        sidePanelSplitPreviewPercent: 33.5,
        sidePanelSplitSubagentPercent: 50,
      },
    });
    expect(service.get("system").sidePanelSplitFilePercent).toBe(60);
  ...
  it("clamps out-of-range split percents to the [22, 78] pane range", async () => {
    ...
    expect(service.get("system").sidePanelSplitFilePercent).toBe(78);
  });
  ...
  it("ignores an invalid split-percent patch and preserves prior value", async () => {
    ...
    expect(service.get("system").sidePanelSplitPreviewPercent).toBe(40);
  });
});
