...
function seed(): ToolRegistry {
  ...
  r.register(makePluginTool("meeting_start", "com.ep.meeting"));
  r.register(makePluginTool("meeting_stop", "com.ep.meeting"));
  r.register(makePluginTool("email_list", "com.ep.email"));
...
describe("ToolRegistry.getToolSchemasForScope — Phase 1 Lazy Tool Scoping", () => {
  ...
  it("single plugin in scope → builtins + that plugin's tools", () => {
    const r = seed();
    const schemas = r.getToolSchemasForScope({
      activePluginIds: new Set(["com.ep.meeting"]),
      includeBuiltins: true,
      includeMcp: false,
    });
    const names = schemas.map((s) => s.name).sort();
  ...
  it("multi-plugin scope → union of plugin tools", () => {
    const r = seed();
    const schemas = r.getToolSchemasForScope({
      activePluginIds: new Set(["com.ep.meeting", "com.ep.email"]),
      includeBuiltins: true,
      includeMcp: false,
    });
    const names = schemas.map((s) => s.name).sort();
