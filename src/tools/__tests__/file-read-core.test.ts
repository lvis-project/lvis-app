...
describe("assertReadableFilePath", () => {
  ...
  it("allows a file inside an extra allowed directory", async () => {
  ...
  it("rejects a symlink whose real target escapes the allowed roots", async () => {
  ...
  it("returns the realpath'd target for an in-scope symlink (guard target === read target)", async () => {
