...
export function createRationaleReviewRequiredRecord(
  ...
  return seal({ contractVersion: RATIONALE_CONTROL_CONTRACT_VERSION,
    ...
    state: "review_required", rationaleStatus: "not-requested", terminalReason: null,
  }, "RationaleTicketStateRecord");
}

export function createRationaleTicketEvent(
  control: RationaleRequiredControl, event: RationaleTicketEventName,
...
export function validateRationaleTicketRecord(record: RationaleTicketStateRecord): void {
  ...
  exact(record, ["contractVersion", "ticketId", "actionDigest", "state",
    "rationaleStatus", "terminalReason"], "RationaleTicketStateRecord");
...
function next(record: RationaleTicketStateRecord, state: RationaleTicketState,
  rationaleStatus: RationaleStatus, terminalReason: RationaleTerminalReason | null,
): RationaleTicketStateRecord {
  return seal({ contractVersion: RATIONALE_CONTROL_CONTRACT_VERSION,
    ticketId: record.ticketId, actionDigest: record.actionDigest,
    state, rationaleStatus, terminalReason }, "RationaleTicketStateRecord");
}
...
export function transitionRationaleTicket(
  ...
  switch (event.event) {
    case "abort": return next(record, "cancelled", record.rationaleStatus, "caller-abort");
    case "session-close":
      return next(record, "cancelled", record.rationaleStatus, "session-close");
    case "identity-mismatch":
      return next(record, "rejected", record.rationaleStatus, "identity-mismatch");
    case "stale-replay":
      return next(record, "rejected", record.rationaleStatus, "stale-replay");
    case "expire": return next(record, "expired", record.rationaleStatus, "expired");
    case "cancel": return next(record, "cancelled", record.rationaleStatus, "user-cancel");
    default: break;
  }
  const key = record.state + ":" + event.event;
  switch (key) {
    case "review_required:request-rationale":
      return next(record, "rationale_requested", "pending", null);
    case "rationale_requested:rationale-ready":
      return next(record, "rationale_ready", "ready", null);
    case "rationale_requested:rationale-failed":
      return next(record, "rationale_failed", "failed", null);
    case "rationale_ready:prompt-user":
    case "rationale_failed:prompt-user":
      return next(record, "user_pending", record.rationaleStatus, null);
    case "user_pending:allow-once":
      return next(record, "allowed_once", record.rationaleStatus, "allowed-once");
    case "user_pending:deny":
      return next(record, "denied", record.rationaleStatus, "user-deny");
    case "user_pending:modal-timeout":
      return next(record, "cancelled", record.rationaleStatus, "modal-timeout");
...
export type InvocationAuditEventName = "start" | "complete" | "fail" | "crash-recovery";
...
const INVOCATION_EVENTS: readonly InvocationAuditEventName[] = [
  "start", "complete", "fail", "crash-recovery",
];

function validateInvocationRecord(record: InvocationAuditRecord): void {
  ...
  exact(record, ["contractVersion", "ticketId", "actionDigest", "invocationDigest",
    "toolUseId", "authorizationReceiptId", "state", "automaticRetry"], "InvocationAuditRecord");
  if (record.contractVersion !== RATIONALE_CONTROL_CONTRACT_VERSION ||
      !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
        .test(record.ticketId) ||
      !/^[0-9a-f]{64}$/.test(record.actionDigest) ||
      ...
      !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
        .test(record.authorizationReceiptId) ||
...
export function createAuthorizedInvocationAudit(input: {
  ...
  return seal({ contractVersion: RATIONALE_CONTROL_CONTRACT_VERSION,
    ...
    toolUseId: input.control.sealedAction.toolUseId, state: "authorized",
...
export function createInvocationAuditEvent(
  ...
  validateInvocationRecord(record);
  return seal({ contractVersion: RATIONALE_CONTROL_CONTRACT_VERSION,
    ticketId: record.ticketId, actionDigest: record.actionDigest,
    invocationDigest: record.invocationDigest,
    authorizationReceiptId: record.authorizationReceiptId, event }, "InvocationAuditEvent");
}
...
export function transitionInvocationAudit(
  ...
  exact(event, ["contractVersion", "ticketId", "actionDigest", "invocationDigest",
    "authorizationReceiptId", "event"], "InvocationAuditEvent");
  if (event.contractVersion !== RATIONALE_CONTROL_CONTRACT_VERSION ||
      event.ticketId !== record.ticketId || event.actionDigest !== record.actionDigest ||
  ...
  const key = record.state + ":" + event.event;
  const transitions: Partial<Record<string, InvocationAuditState>> = {
    "authorized:start": "started", "started:complete": "completed",
    "started:fail": "failed", "started:crash-recovery": "unknown-after-crash",
  };
  const state = transitions[key];
  if (!state) throw new Error("invalid invocation audit transition: " + key);
  return seal({ ...record, state }, "InvocationAuditRecord");
}
