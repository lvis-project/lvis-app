 ...
 *     is async, so synchronous methods are never converted to async. hostFetch
 *     is the lone VERB-derived chokepoint: its read/write class
 ...
 *   • The TWO remaining exclusions run UNGATED under the flag, each BOUNDED:
 ...
 */
function descriptorKey(pluginId: string, methodPath: string, target: string | undefined,
): string {
...
export async function gateMutatingEffect(params: {
  ...
  const where = params.target
    ? `${params.methodPath} → ${params.target}`
    : params.methodPath;
