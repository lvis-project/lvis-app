 ...
 *
 * Covers:
 *   (a) only one-shot decision choices reach the caller
...
describe("requestAgentApproval", () => {
  ...
  it("(a) clamps a non-one-shot allow response to deny-once", async () => {
    const gate = makeMockGate({
      requestId: "test-id",
      choice: "allow-always",
      nonce: "aabbcc",
  ...
  it("(b) nonce/HMAC are NOT supplied by the caller — only gate fields reach requestAndWait", async () => {
    ...
    expect(calledReq.category).toBe("agent-action");
    expect(calledReq.kind).toBe("agent-action");
    expect(calledReq.allowedChoices).toEqual(["allow-once", "deny-once"]);
    expect(calledReq.durableApprovalRecordAllowed).toBe(false);
    ...
    expect(calledReq.sourcePluginId).toBe(BASE_INPUT.sourcePluginId);
    expect(calledReq.approvalScope).toBe(BASE_INPUT.scope);
