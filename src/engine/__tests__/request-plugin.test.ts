...
function makeLoop(opts: {
  ...
  // a plugin tool gated by com.ep.meeting scope
  toolRegistry.register(createDynamicTool({
    name: "meeting_start",
    description: "회의 시작",
    source: "plugin",
    pluginId: "com.ep.meeting",
...
describe("ConversationLoop — request_plugin meta tool (Option C)", () => {
  it("activates plugin and exposes its tool in the next round", async () => {
    const provider = new RecordingProvider([
      ...
      [
        { type: "tool_call", id: "tu-1", name: "request_plugin", input: { pluginId: "com.ep.meeting" } },
    ...
    const loop = makeLoop({ provider, availablePluginIds: ["com.ep.meeting"] });
  ...
  it("rejects request_plugin outside the loop allowedPluginIds scope", async () => {
    ...
    const loop = makeLoop({
      provider,
      availablePluginIds: ["com.ep.meeting"],
      allowedPluginIds: [],
    });
  ...
  it("keeps forced plugins active even when allowedPluginIds is deny-all", async () => {
    ...
    const loop = makeLoop({
      ...
      availablePluginIds: ["com.ep.meeting"],
      allowedPluginIds: [],
      forcedActivePluginIds: ["com.ep.meeting"],
    });
