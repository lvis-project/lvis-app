...
function settledAttemptWithExtension(extensionUri: string) {
...
describe("A2A remote startup recovery fence", () => {
  it("migrates the domain-shaped v2 replay identifier to the reviewed UUID URN", async () => {
    const legacyIdentifier = Buffer.from(
      "aHR0cHM6Ly9sdmlzLmFpL2EyYS9leHRlbnNpb25zL2V4YWN0LXNlbmQtcmVwbGF5L3Yx",
      "base64",
    ).toString("utf8");
    const { current, record } = settledAttemptWithExtension(legacyIdentifier);
  ...
  it("quarantines an arbitrary v2 lookalike instead of promoting it to the trusted URN", async () => {
    const lookalike = new URL(
      "/a2a/extensions/exact-send-replay/v1",
      `https://${["arbitrary", "example", "test"].join(".")}`,
    ).href;
    const { current, record } = settledAttemptWithExtension(lookalike);
    const files = new Map<string, unknown>([["client-state.json", {
      version: 2, attempts: [record], payloads: [], tasks: [],
    }]]);
    const audit = vi.fn();
    const store = new A2ARemoteDurableStore({
      namespace: {
    ...
    expect(await store.getAttempt(current.attemptId)).toBeNull();
    expect(prepareSecret).not.toHaveBeenCalled();
    expect(resolve).not.toHaveBeenCalled();
    expect(invoke).not.toHaveBeenCalled();
    expect(audit).toHaveBeenCalledWith({ reason: "startup-quarantine", count: 1 });
    expect(files.get("client-state.quarantine.json")).toMatchObject({ version: 1, entries: [{ reason: "attempt-schema-invalid" }] });
  });
