 ...
 * `text/event-stream` and subscribed to the {@link StreamEventSource}. Each
...
export interface LocalApiHttpServerOptions {
  ...
  /**
   * Bind host. Must be an IPv4 or IPv6 loopback literal (for example
   * `127.0.0.1` or `::1`); hostnames and non-loopback addresses are rejected.
   */
  host?: string;
...
/**
 * Whether `host` is a literal loopback address that node can bind without a
 * DNS lookup. Keeping this a literal-only allowlist makes the loopback promise
 * true at bind time: a hostname must not be able to resolve to a Tailnet or
 * other non-local address between validation and `listen()`.
 */
function isLoopbackBindHost(host: string): boolean {
  const family = isIP(host);
  if (family === 4) {
    // 127.0.0.0/8 is the IPv4 loopback block, not merely 127.0.0.1.
    return Number(host.split(".", 1)[0]) === 127;
  }
  if (family !== 6) return false;

  // `isIP` has already validated the syntax. Expand an optional `::` so all
  // RFC-valid spellings of the one IPv6 loopback address are accepted, while
  // unspecified (`::`) and IPv4-mapped addresses remain fail-closed.
  ...
  return groups.length === 8
    && groups.slice(0, -1).every((group) => Number.parseInt(group, 16) === 0)
    && Number.parseInt(groups[groups.length - 1] ?? "", 16) === 1;
}
...
function handleEvents(
  ...
  // Assigned before it is added to `liveStreams`; the placeholder lets cleanup
  // safely run even if the peer closes while the SSE preamble is being written.
  let endStream: () => void = () => {};
  ...
  const unsubscribe = broadcaster.subscribe((channel, payload) => {
    // `payload` may contain newlines only via JSON escaping, so one `data:` line
    // is sufficient and correct. A malformed/unserializable display payload is
    // skipped; it must not throw back into the model-facing broadcaster.
    ...
    writeOrClose(`event: ${channel}\ndata: ${serialized}\n\n`);
  });
  // Defend the cleanup invariant even if a future broadcaster synchronously
  // replays an event during subscribe().
  if (cleaned) {
    unsubscribe();
    return;
  }
...
export function startLocalApiHttpServer(
  ...
  if (!isLoopbackBindHost(host)) {
    return Promise.reject(new Error("local-api http server host must be a literal loopback address"));
  }
