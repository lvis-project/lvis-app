...
app.setAppUserModelId("com.ep.lvis");
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // sandbox:false is required so the preload can use Node built-ins
      // (node:path, node:url) that PR #273 introduced. contextIsolation:true
      // still ensures the renderer cannot access Node APIs directly.
      sandbox: false,
