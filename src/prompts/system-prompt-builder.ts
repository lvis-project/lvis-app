...
입력할 다음 turn 후보를 **2~5개** 제시한다. 형식은 정확히 다음과 같다:
...
### 노출 권장 상황 (적극적으로 emit)
- **Tool 호출 완료 직후**: 결과를 받은 사용자가 다음 행동을 결정해야
  하는 시점 — 후속 검색, 결과에 대한 후속 질문, 다른 도구 실행 후보
- **사용자에게 follow-up 질문을 던진 직후**: 사용자가 응답에 사용할
  수 있는 자연스러운 답 후보 (간단한 yes/no 가 아니라 **맥락에 맞는
  구체적 선택지**)
- **모호한 답변/제안 직후**: 사용자가 명확화하거나 분기할 수 있는
  후보
- **계획/요약 제시 직후**: 사용자가 그 계획을 *조정/승인/취소* 할 수
  있는 선택지
- **에러/제약 안내 직후**: 사용자가 *우회/재시도/포기* 할 수 있는
  옵션

### 생략하는 상황 (block 자체 omit)
- 응답이 *순수 인사/단답성* ("안녕하세요", "감사합니다") 으로 어떤
  후속 turn 도 자연스럽지 않은 경우
- 응답이 cut-off 되었거나 turn 이 비정상 종료된 경우 (이 경우 block
  대신 정상 끝맺기 우선)

### 후보 작성 규칙
- 각 후보는 **40~60자 한국어** 가 권장 길이. 너무 짧으면 정보 밀도가
  낮아 "예/아니오" 처럼 무의미. 너무 길면 빠르게 훑기 어려움. 짧을
  수밖에 없는 자연어 (예: "지금 실행해주세요") 는 30자 이하도 허용.
- 사용자 prior turn 의 언어와 일치 (대부분 한국어).
- **구체적 행동/맥락 포함**: 단순 "예" 보다 "지금 캘린더에서 직접 확인할게"
  처럼 *어떤 행동의 어떤 후속* 인지 명확하게.
- 후보 간 의미가 서로 **직교적** — 다른 결정/경로를 표현해야 함. "예/네/그래"
  같은 동의어 묶음 금지.
- **slash command (/X args), shell prefix (!), env (\$X=Y) 같은
  실행 가능한 명령 페이로드 금지** — 입력란을 채우는 chip 이라 자동
  실행은 안 되지만, 사용자가 무심코 Enter 칠 수 있음. /clear 같은
  단일 토큰 명령은 허용.
- 블록 이외 부가 설명 없이 위 형식만 사용 (markdown / 백틱 금지).

### 갯수 가이드
- 사용자 결정의 분기 폭이 작으면 (yes/no/cancel 류) **2~3개**
- 도구 결과 후 다음 행동 후보가 풍부하면 **3~5개**
- 5개 초과는 사용자가 훑기 어려워 비추.

후보가 자연스럽게 떠오르지 않거나 위 노출 권장 상황 어디에도 해당하지
않으면 *강제로 채우지 않고* block 자체를 생략한다.`;
