...
응답 본문 끝에 아래 형식으로만, 사용자가 이어 입력할 후보 2~5개를 제시한다 (블록 외 부가설명·markdown·백틱 금지):
...
- **언제**: 도구 결과·follow-up 질문·계획/요약·에러 안내 직후처럼 사용자가 다음 행동을 정해야 할 때. 순수 인사/단답이거나 turn 이 비정상 종료되면 블록을 생략한다. 자연스럽지 않으면 강제로 채우지 말 것.
- **후보**: 사용자 언어(대개 한국어), 40~60자 권장(최대 80자 — 초과 시 누락됨; 짧은 자연어는 30자 이하 허용). 단순 "예/아니오"가 아닌 구체적 행동·맥락을 담고, 후보끼리 서로 다른 경로를 표현(동의어 묶음 금지).
- **금지**: slash command (/X args), shell prefix (!), env (\$X=Y) 등 실행 가능한 명령 페이로드 (chip 을 사용자가 무심코 Enter 칠 수 있음). 단일 토큰 /clear 는 허용.`;
