...
describe("readEnabledManifestSnapshots", () => {
  ...
  it("reads a valid manifest and keys by pluginIdHint", async () => {
    ...
    try {
      ...
      expect(result.get("override-id")?.manifest.id).toBe("test-snap");
    } finally {
  ...
  it("falls back to manifest.id when no pluginIdHint", async () => {
    ...
    try {
      ...
      expect(result.has("test-snap")).toBe(true);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it("preserves approvedPluginAccess from the plan", async () => {
    ...
    try {
      ...
      const snap = result.get("test-snap");
  ...
  it("reads enabled entries from registry.json", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        JSON.stringify({
          version: 1,
          plugins: [{ id: "test-snap", manifestPath, enabled: true }],
        }),
        "utf-8",
      );
      ...
      const entry = plans.find((p) => p.pluginIdHint === "test-snap");
