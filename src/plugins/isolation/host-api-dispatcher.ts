 ...
 * entries in it. The factories used to sit in three sibling files split by
 * category — interaction, service, storage — but the categories were a filing
 * label, not a boundary: all three build `HostApiPathHandler`s out of
 * {@link defineHostApiPath}, all three are spread over this one table in a
 * single expression in `out-of-process-plugin.ts`, and nothing else imports any
 * of them. The split that IS real is host vs child, because it is two processes.
 *
