...
describe("PluginMarketplaceService install → update → rollback", () => {
  ...
  beforeEach(async () => {
    ...
    pluginDir = join(installedDir, "com.ep.sample");
  ...
  function makeService() {
    ...
    vi.spyOn(svc as unknown as {
      ...
      await writeFile(
        manifestFile,
        JSON.stringify({ id: "com.ep.sample", version, entry: "./dist/index.js", tools: [] }),
        "utf-8",
      );
      return "com.ep.sample/plugin.json";
    });
    return svc;
  }
  ...
  it("rollback without a prior cached version throws", async () => {
    const svc = makeService();
    await svc.installPlugin("com.ep.sample", "1.0.0");
    await expect(svc.rollbackPlugin("com.ep.sample")).rejects.toThrow(/No prior version/);
  });
  ...
  it("installPlugin clears _devLinked on same-version fast-path (touchInstalledRegistryEntry)", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        version: 1,
        plugins: [{ id: "com.ep.sample", manifestPath: "com.ep.sample/plugin.json", enabled: true, installedBy: "user", _devLinked: true }],
      }),
      "utf-8",
    );
    const svc = makeService();
    ...
    await svc.installPlugin("com.ep.sample", "1.0.0");
    await svc.installPlugin("com.ep.sample", "1.0.0"); // same version → fast path
  ...
  it("rollbackPlugin clears _devLinked on the rolled-back entry", async () => {
    ...
    await svc.rollbackPlugin("com.ep.sample");
  ...
  it("installPlugin sets installSource='user' when overwriting a dev-link entry", async () => {
    await writeFile(
      ...
      JSON.stringify({
        version: 1,
        plugins: [{ id: "com.ep.sample", manifestPath: "com.ep.sample/plugin.json", enabled: true, installedBy: "user", _devLinked: true, installSource: "dev-link" }],
      }),
      "utf-8",
    );
    const svc = makeService();
  ...
  it("rollback fails with explicit delisted-plugin message when catalog no longer lists the id", async () => {
    ...
    await expect(svc.rollbackPlugin("com.ep.sample")).rejects.toThrow(
