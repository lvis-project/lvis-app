...
`,
  ...
  it("boot-loaded plugin methods are not callable until startAll completes start()", async () => {
    ...
    await expect(runtime.call("p_start_guard_ping")).rejects.toThrow(
      /not found|not active|still starting/,
    );
...
// ─── Lifecycle logPluginLifecycle emission smoke tests ─────────────────────────────────────

describe("PluginRuntime lifecycle logPluginLifecycle emission", () => {
  let testDir: string;
  let installedDir: string;
  let registryPath: string;

  beforeEach(async () => {
    testDir = mkdtempSync(join(tmpdir(), "lvis-logPluginLifecycle-"));
    installedDir = join(testDir, "plugins");
    await mkdir(installedDir, { recursive: true });
    registryPath = join(installedDir, "registry.json");
  });
  ...
  it("emits LOAD_START phase for each plugin entry during load()", async () => {
    const manifestPath = await writeFakePlugin("logPluginLifecycle-test");
    await writeTestPluginRegistry({ registryPath }, [
      { id: "logPluginLifecycle-test", manifestPath, enabled: true },
    ]);
  ...
  it("emits LOAD_OK phase after a plugin successfully loads", async () => {
    const manifestPath = await writeFakePlugin("logPluginLifecycle-ok");
    await writeTestPluginRegistry({ registryPath }, [
      { id: "logPluginLifecycle-ok", manifestPath, enabled: true },
    ]);
    const calls: unknown[][] = [];
    const spy = vi.spyOn(console, "log").mockImplementation((...args) => {
      calls.push(args);
  ...
  it("emits RESTART_REQUEST phase when restartPlugin is called", async () => {
    const manifestPath = await writeFakePlugin("logPluginLifecycle-restart");
    await writeTestPluginRegistry({ registryPath }, [
      { id: "logPluginLifecycle-restart", manifestPath, enabled: true },
    ]);
    const runtime = makeGenerationBoundRuntime({
      hostRoot: testDir,
      registryPath,
    ...
    await runtime.restartPlugin("logPluginLifecycle-restart");
