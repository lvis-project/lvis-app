...
describe("renderer PageIndex capability lookup", () => {
  it("finds tool from plugin with knowledge-index capability", () => {
    const cards: PluginCard[] = [
      { id: "com.ep.email", name: "Email", description: "", sampleTools: [], tools: ["email_list"], capabilities: ["email-reader"] },
      { id: "com.ep.pageindex", name: "PageIndex", description: "", sampleTools: [], tools: ["page_index_list_documents", "page_index_search"], capabilities: ["knowledge-index"] },
    ];
  ...
  it("returns undefined when knowledge-index plugin has no matching tool", () => {
    const cards: PluginCard[] = [
      { id: "com.ep.pageindex", name: "PageIndex", description: "", sampleTools: [], tools: ["page_index_search"], capabilities: ["knowledge-index"] },
    ];
    expect(findListDocumentTool(cards)).toBeUndefined();
  });
  ...
  it("matches pageindex `index_documents` style (no 'list' verb)", () => {
    const cards: PluginCard[] = [
      { id: "com.ep.pageindex", name: "PageIndex", description: "", sampleTools: [], tools: ["index_documents", "index_search"], capabilities: ["knowledge-index"] },
    ];
  ...
  it("matches alternative tool name patterns", () => {
    const cards: PluginCard[] = [
      { id: "com.ep.pageindex", name: "PageIndex", description: "", sampleTools: [], tools: ["pageindex_list_documents"], capabilities: ["knowledge-index"] },
    ];
