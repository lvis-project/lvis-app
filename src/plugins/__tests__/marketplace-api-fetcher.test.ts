/**
 * Tests for MarketplaceApiFetcher — §9.5 M4.
 *
...
import { MarketplaceApiFetcher } from "../marketplace-api-fetcher.js";
...
describe("MarketplaceApiFetcher (public-network path)", () => {
  beforeEach(() => {
  ...
  it("listPlugins() parses a server catalog response", async () => {
    ...
    const fetcher = new MarketplaceApiFetcher({
...
describe("MarketplaceApiFetcher (private-network path)", () => {
...
describe("MarketplaceApiFetcher — actual server response shape", () => {
  const originalFetch = global.fetch;
...
describe("MarketplaceApiFetcher — input validation (security)", () => {
  beforeEach(() => {
    mockedFetchPublic.mockReset();
  });
