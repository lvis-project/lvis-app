...
describe("PluginRuntime.listPluginCards — Phase 1.5 Option C catalog", () => {
  ...
  it("returns id, name, description, sampleTools (max 3)", async () => {
    const manifestA = writePlugin(tmp, "com.ep.meeting", {
      name: "Meeting",
    ...
    expect(cards[0].id).toBe("com.ep.meeting");
  ...
  it("falls back to 'Plugin: {name}' when toolSchemas absent", async () => {
    const manifestA = writePlugin(tmp, "com.ep.plain", {
      name: "Plain",
      tools: ["plain_do"],
    });
  ...
  it("MEDIUM-1: sampleTools excludes deny-rule-blocked tools when toolRegistry provided", async () => {
    const manifestA = writePlugin(tmp, "com.ep.filtered", {
      name: "Filtered",
  ...
  it("surfaces disabled and failed plugin loadStatus values from registry-backed runtime", async () => {
    const disabledManifest = writePlugin(tmp, "com.ep.disabled", {
      name: "Disabled",
      tools: ["disabled_read"],
    });
    const failedDir = join(tmp, "com.ep.failed");
    ...
    const registryPath = writeRegistry(tmp, [
      { id: "com.ep.disabled", manifestPath: disabledManifest, enabled: false },
      { id: "com.ep.failed", manifestPath: join(failedDir, "plugin.json"), enabled: true },
    ]);
    ...
    expect(cards).toEqual([
      expect.objectContaining({ id: "com.ep.disabled", loadStatus: "disabled" }),
      expect.objectContaining({ id: "com.ep.failed", loadStatus: "failed" }),
    ]);
  });
});
