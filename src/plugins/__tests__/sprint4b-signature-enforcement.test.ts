...
describe("Sprint 4-B — signature enforcement", () => {
  ...
  it("fail-closed: managed plugin without signature is dropped", async () => {
    await writePlugin("com.ep.managed", "managed", false);
    ...
    await runtime.load();
    expect(runtime.listPluginIds()).not.toContain("com.ep.managed");
  });

  it("managed plugin with valid signature loads", async () => {
    await writePlugin("com.ep.managed-signed", "managed", true);
    const verifier = new PluginSignatureVerifier({ publisherPublicKeysPem: [publicKeyPem] });
    const runtime = new PluginRuntime({
      hostRoot: testDir,
      registryPath,
    ...
    await runtime.load();
    expect(runtime.listPluginIds()).toContain("com.ep.managed-signed");
  });
