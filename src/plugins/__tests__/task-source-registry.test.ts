...
describe("TaskSourceRegistry", () => {
  ...
  it("plugin can enrich legacy seed with label", () => {
    const reg = new TaskSourceRegistry();
    reg.register({ id: "email", origin: "plugin", pluginId: "com.ep.email", label: "이메일" });
...
describe("deriveCategoryId", () => {
  it("uses explicit source when provided", () => {
    expect(deriveCategoryId("com.ep.email", "email-task")).toBe("email-task");
  });

  it("derives from last dotted segment of pluginId", () => {
    expect(deriveCategoryId("com.ep.meeting-recorder", undefined)).toBe("meeting-recorder");
  });
  ...
  it("trims whitespace from explicit source", () => {
    expect(deriveCategoryId("com.ep.email", "  email  ")).toBe("email");
  });

  it("ignores empty string explicit source", () => {
    expect(deriveCategoryId("com.ep.email", "")).toBe("email");
  });
  ...
  it("does not throw when explicitSource is an object (falls back to pluginId)", () => {
    ...
    expect(() =>
      deriveCategoryId("com.ep.email", { x: 1 } as unknown as string),
    ).not.toThrow();
    expect(deriveCategoryId("com.ep.email", { x: 1 } as unknown as string)).toBe("email");
  });

  it("does not throw when explicitSource is a number (falls back to pluginId)", () => {
    expect(() =>
      deriveCategoryId("com.ep.email", 42 as unknown as string),
    ).not.toThrow();
    expect(deriveCategoryId("com.ep.email", 42 as unknown as string)).toBe("email");
  });

  it("ignores null explicitSource", () => {
    expect(deriveCategoryId("com.ep.email", null as unknown as string)).toBe("email");
  });
});
