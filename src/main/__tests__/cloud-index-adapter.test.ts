/**
 * DisabledCloudIndexAdapter — 빈 결과 계약 검증
 *
 * 청사진 §6.1: `lvis-app/src/main/cloud-index-adapter.ts`
 * 청사진 §1 C4: cloud weight = 0.0 (cloud leg contributes nothing)
 *
 * 실행: npm test (vitest)
 */
...
describe("DisabledCloudIndexAdapter", () => {
  it("search() returns empty array (any query)", async () => {
    const adapter = new DisabledCloudIndexAdapter();
