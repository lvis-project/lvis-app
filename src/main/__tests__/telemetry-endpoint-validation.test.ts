...
describe("validateTelemetryEndpoint — schemes", () => {
  it("accepts https://", () => {
    expect(
      validateTelemetryEndpoint("https://telemetry.portal.example.com/ingest", {
        isPackaged: true,
      }).valid,
    ).toBe(true);
  });
  ...
  it("rejects http://", () => {
    const r = validateTelemetryEndpoint("http://telemetry.portal.example.com/ingest", {
      isPackaged: true,
    });
...
describe("validateTelemetryEndpoint — allowlist", () => {
  it("accepts telemetry.portal.example.com by default", () => {
    expect(
      validateTelemetryEndpoint("https://telemetry.portal.example.com/x", { isPackaged: true }).valid,
    ).toBe(true);
  });
  ...
  it("env override fully replaces default (e.g. telemetry.portal.example.com becomes rejected)", () => {
    const r = validateTelemetryEndpoint("https://telemetry.portal.example.com/x", {
      isPackaged: true,
  ...
  it("sanity — DEFAULT_TELEMETRY_ALLOWLIST exposes the documented hosts", () => {
    expect(DEFAULT_TELEMETRY_ALLOWLIST).toContain("telemetry.portal.example.com");
...
describe("TelemetryService — isActive() + audit on invalid endpoint", () => {
  ...
  it("isActive() is false when endpoint is http://", () => {
    ...
    const svc = new TelemetryService({
      settings: makeSettings({ endpoint: "http://telemetry.portal.example.com/x" }),
      isPackaged: true,
      auditLogger: logger,
    });
