 ...
 * CorporateRootCA를 신뢰하지 않으면 api.openai.com 등 모든 외부 HTTPS 호출이
 ...
 *   [3] CN=CorporateRootCA issuer=CorporateRootCA (self-signed, 2005-2045)
 ...
 *   - Windows: `certutil -exportPFX -p "" Root "CorporateRootCA" tmp.pfx` or win-ca npm pkg
 *   - Linux:   /etc/ssl/certs/CorporateRootCA*.pem or `update-ca-certificates` hook
 */
 ...
 * MDM이 CorporateRootCA를 배포하므로 root CA 하나만 추출하면 chain이 해결된다.
 */
const corporate_ROOT_CA_CN = "CorporateRootCA";
...
function extractMacos(): string | null {
  try {
    const pem = execSync(
      `security find-certificate -a -c '${corporate_ROOT_CA_CN}' -p /Library/Keychains/System.keychain`,
      { encoding: "utf8", timeout: 10_000 },
    );
    if (!pem.includes("-----BEGIN CERTIFICATE-----")) {
      console.warn("[corp-ca] macOS: CorporateRootCA not found in System.keychain");
      return null;
    }
    return pem;
  } catch (err) {
...
function extractLinux(): string | null {
  ...
  // /etc/ssl/certs/CorporateRootCA.pem 또는 update-ca-trust 경로 탐색
