...
describe("fanOutToAllWindows", () => {
  ...
  it("emits one info audit row with channel + delivered count", () => {
    ...
    fanOutToAllWindows([w1, w2] as never, "lvis:test", undefined, {
      auditLogger,
    });
    ...
    expect(auditLogger.log).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "info",
        sessionId: "ipc",
        input: "[broadcast] channel=lvis:test delivered=2/2",
      }),
    );
  });
  ...
  it("does not emit an audit row when no auditLogger is supplied", () => {
    // Smoke check that a logger-less, audit-less broadcast is a no-op
