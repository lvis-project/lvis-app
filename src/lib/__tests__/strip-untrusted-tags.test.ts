...
describe("stripUntrustedTags", () => {
  ...
  it("strips multiple wrap classes", () => {
    const input =
      "<untrusted-subject>제목</untrusted-subject> | <untrusted-sender-domain>finance.portal.example.com</untrusted-sender-domain>";
    expect(stripUntrustedTags(input)).toBe("제목 | finance.portal.example.com");
  });
