...
export function SlashPicker({
  ...
  const openMenu = useCallback(async () => {
    ...
    // released now. It exists to let ⌘K raise the menu, not to track it.
    onOpenChange(false);
  }, [actions, plugins, onInsert, onSelectPlugin, onRunMcpPrompt, showMenu, onOpenChange]);

  // ⌘K sets the flag; raising the menu is the same path as a click.
  const openRef = useRef(open);
  if (open && !openRef.current) void openMenu();
