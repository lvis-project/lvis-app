...
} from "../hooks/use-plugin-marketplace.js";
import type {
  LvisApi,
  MarketplaceItem,
  PluginCardSummary,
...
function formatInstallProgress(progress: InstallProgressPayload): string {
  if (progress.phase !== "downloading")
    return t(INSTALL_PHASE_I18N_KEY[progress.phase]);
  ...
  return t("pluginConfigTab.downloadingBytesOf", {
    received,
...
function findRefreshedDoctorPlugin(
  ...
  const lookupKeys = [
    plugin.id,
    ...(plugin.installAliases ?? []),
    ...additionalKeys,
  ];
  return cards.find((card) =>
    lookupKeys.some((key) =>
      isPluginInstallKey(card.id, key, card.installAliases),
    ),
  );
}
...
async function resolvePluginDoctorInstallKey(
  plugin: PluginCardSummary,
): Promise<string> {
...
function PluginPreparationStatusPanel({
  plugin,
}: {
  plugin: PluginCardSummary;
}) {
  ...
  const phaseLabel = status?.phase
    ? PREPARATION_PHASE_I18N_KEY[status.phase]
      ? t(PREPARATION_PHASE_I18N_KEY[status.phase])
      : status.phase
    : t("pluginConfigTab.prepPhasePending");
  const message = status?.message ?? t("pluginConfigTab.preparingRuntime");
  return (
    <div
      className="space-y-2 rounded-sm bg-warning/(--opacity-subtle) px-3 py-2"
      aria-live="polite"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 space-y-0.5">
          <div className="text-[10px] font-medium uppercase text-warning">
            {t("pluginConfigTab.preparationStatusLabel")}
          </div>
        ...
        {progressPct !== null && (
          <div className="shrink-0 font-mono text-[11px] text-warning tabular-nums">
            {progressPct}%
          </div>
        )}
      </div>
      <div className="h-1.5 w-full overflow-hidden rounded-full bg-warning/(--opacity-light)">
        ...
        <div
          ...
          style={
            progressPct === null
              ? undefined
              : ({ "--progress": `${progressPct}%` } as CSSProperties)
          }
        />
      </div>
...
export function PluginConfigTab({ api }: { api?: LvisApi } = {}) {
  ...
  const [uninstallTarget, setUninstallTarget] =
    useState<PluginCardSummary | null>(null);
  ...
  const { statuses: authStatuses, refresh: refreshAuthStatus } =
    usePluginAuthStatuses(apiForAuthHook, plugins);
  ...
  const refreshPlugins = useCallback(
    async (options?: {
      ...
      try {
        ...
        showBanner(
          "error",
          (e as Error).message ?? t("pluginConfigTab.errorLoadPlugins"),
        );
        return null;
      } finally {
        setLoading(false);
    ...
    async (pluginId: string, nextEnabled: boolean) => {
      ...
      try {
        ...
        if (!res.ok) {
          showBanner(
            "error",
            res.message ?? t("pluginConfigTab.errorToggleEnabled"),
          );
          return;
        }
        ...
        showBanner(
          "error",
          (e as Error).message ?? t("pluginConfigTab.errorToggleEnabled"),
        );
      } finally {
        setTogglingId(null);
      }
    ...
    (async () => {
      try {
        ...
        if (!result.ok) {
          ...
          showBanner(
            "error",
            result.message ?? t("pluginConfigTab.errorLoadConfig"),
          );
          return;
        }
        ...
        if (!cancelled)
          showBanner(
            "error",
            (e as Error).message ?? t("pluginConfigTab.errorLoadConfig"),
          );
      }
    })();
    return () => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      showBanner(
        "error",
        (e as Error).message ?? t("pluginConfigTab.errorSave"),
      );
    } finally {
      setSaving(false);
    }
  ...
  const selectedPluginActive = selectedPlugin
    ...
    : false;
  const selectedPluginRuntimeLoaded = selectedPlugin
    ? isRuntimeCallablePlugin(selectedPlugin)
    : false;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result =
          await window.lvis.pluginConfig.listSecretKeys(selectedId);
        if (cancelled) return;
  ...
  const handleInstallLocal = useCallback(async () => {
    ...
    try {
      ...
      showBanner(
        "success",
        t("pluginConfigTab.successLocalInstall", { pluginId: result.pluginId }),
      );
      void refreshPlugins();
    } catch (e) {
      showBanner(
        "error",
        (e as Error).message ?? t("pluginConfigTab.errorLocalInstall"),
      );
    } finally {
  ...
  const handleDoctorPlugin = useCallback(
    async (plugin: PluginCardSummary) => {
      ...
      try {
        ...
        if (!isReinstallFixableFailureKind(plugin.installFailureKind)) {
          ...
          if (isDoctorRepairLoaded(refreshedPlugin)) {
            showBanner(
              "success",
              t("pluginConfigTab.successDoctor", { displayName: plugin.name }),
            );
          } else {
            showBanner(
              "warning",
              t("pluginConfigTab.doctorDiagnosticUnresolved", {
        ...
        const networkAccessAcknowledgement = buildNetworkAccessAcknowledgement(
          plugin.networkAccess,
        );
        const result = networkAccessAcknowledgement
          ? await getHostMarketplaceApi().installMarketplacePlugin(
              installKey,
              undefined,
              {
        ...
        if (!result.ok) {
          ...
          showBanner(
            "error",
            result.message ?? result.error ?? t("pluginConfigTab.errorDoctor"),
          );
          return;
        }
        ...
        const refreshedCards = await refreshPlugins({
          preferInstallKey: preferredInstallKey,
        });
        if (!refreshedCards) return;
        const refreshedPlugin = findRefreshedDoctorPlugin(
          refreshedCards,
        ...
        showBanner(
          "error",
          (e as Error).message ?? t("pluginConfigTab.errorDoctor"),
        );
      } finally {
        setDoctoringId(null);
      }
  ...
  const handleUninstall = useCallback(
    async (pluginId: string, displayName: string) => {
      setSaving(true);
      try {
        const doctorCleanupKind =
          uninstallTarget?.id === pluginId
            ? uninstallTarget.installFailureKind
            : undefined;
        ...
        const result = uninstallOptions
          ? await marketplaceApi.uninstallMarketplacePlugin(
              pluginId,
              uninstallOptions,
            )
        ...
        if (!result.ok) {
          showBanner(
            "error",
            result.message ?? t("pluginConfigTab.errorUninstall"),
          );
          return;
        }
        ...
        showBanner(
          "success",
          t("pluginConfigTab.successUninstall", { displayName }),
        );
      } catch (e) {
        showBanner(
          "error",
          (e as Error).message ?? t("pluginConfigTab.errorUninstall"),
        );
      } finally {
        setSaving(false);
          ...
          <div className="flex min-w-0 flex-col gap-2 rounded-md border border-warning/(--opacity-medium) bg-warning/(--opacity-soft) px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0 space-y-0.5">
              <p className="text-xs font-medium text-warning">
                {t("pluginConfigTab.devToolsTitle")}
              </p>
            ...
            >
              {localInstalling
                ...
                : t("pluginConfigTab.installFromLocalFolder")}
            </Button>
          </div>
        )}
          ...
          <div className="flex min-w-0 flex-col gap-3 sm:h-[calc(100dvh-180px)] sm:min-h-[350px] sm:flex-row">
                ...
                <div className="p-1 space-y-0.5">
                  {plugins.map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-1 min-w-0">
                        {p.isManaged && (
                          <span title={t("pluginConfigTab.managedPluginTitle")}>
                            🔒
                          </span>
                        )}
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        {p.auth &&
                          isRuntimeCallablePlugin(p) &&
                          authStatuses.get(p.id)?.kind === "unauthed" && (
                            <span
                              className="inline-block rounded-full bg-destructive/(--opacity-soft) px-1.5 py-px text-[9px] font-medium text-destructive"
                  ...
                  {Object.entries(installInFlight)
                    .filter(
                      ([slug]) =>
                        !plugins.some((p) =>
                          isPluginInstallKey(p.id, slug, p.installAliases),
                        ),
                    )
                    .map(([slug, progress]) => (
                      <div
                        ...
                        aria-label={t("pluginConfigTab.installingAriaLabel", {
                          slug,
                        })}
                        aria-live="polite"
                      >
            ...
            <div className="min-w-0 flex flex-col gap-2 rounded-md border bg-card p-3 overflow-visible sm:flex-1 sm:min-h-0 sm:overflow-y-auto">
              {selectedPlugin ? (
                <>
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-1">
                        ...
                        {selectedPlugin.isManaged && (
                          <span className="inline-flex items-center gap-0.5 rounded-full bg-info/(--opacity-soft) px-1.5 py-px text-[9px] font-medium text-info">
                            🔒 {t("pluginConfigTab.badgeManaged")}
                          </span>
                        )}
                        ...
                        {selectedPlugin.installPolicy === "user" && (
                          <span
                            ...
                            aria-label={t(
                              "pluginConfigTab.userInstallableAriaLabel",
                            )}
                          >
                    ...
                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                        ...
                        <label className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                          <span>
                            {selectedPluginActive
                              ? t("pluginConfigTab.activeLabel")
                              : t("pluginConfigTab.inactiveLabel")}
                          </span>
                          <Switch
                            size="sm"
                            ...
                            onCheckedChange={(next) =>
                              void handleToggleEnabled(selectedPlugin.id, next)
                            }
                            aria-label={t("pluginConfigTab.toggleAriaLabel", {
                      ...
                      {selectedPlugin.loadStatus === "failed" && (
                        <Button
                          ...
                          onClick={() =>
                            void handleDoctorPlugin(selectedPlugin)
                          }
                      ...
                      <Button
                        ...
                        title={
                          selectedPlugin.isManaged
                            ? t("pluginConfigTab.managedCannotRemoveTitle")
                            : undefined
                        }
                      >
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 space-y-0.5">
                          <p className="text-xs font-medium text-destructive">
                            {selectedPlugin.installFailureKind ===
                            "catalog-grant-mismatch"
                              ? t("pluginConfigTab.doctorGrantMismatchTitle")
                              : selectedPlugin.installFailureKind ===
                                  "incompatible-app-version"
                                ? t(
                                    "pluginConfigTab.doctorIncompatibleAppTitle",
                                  )
                                : selectedPlugin.installFailureKind ===
                                    "manifest-validation-error"
                                  ? t(
                                      "pluginConfigTab.doctorManifestValidationTitle",
                                    )
                          ...
                          <p className="text-[11px] text-destructive/(--opacity-intense)">
                            ...
                            "catalog-grant-mismatch"
                              ? t(
                                  "pluginConfigTab.doctorGrantMismatchDescription",
                                )
                              : selectedPlugin.installFailureKind ===
                                  "incompatible-app-version"
                                ? t(
                                    "pluginConfigTab.doctorIncompatibleAppDescription",
                                  )
                                : selectedPlugin.installFailureKind ===
                                    "manifest-validation-error"
                                  ? t(
                                      "pluginConfigTab.doctorManifestValidationDescription",
                                    )
                    ...
                    apiForAuthHook && (
                      <>
                        ...
                        <PluginAuthSection
                          ...
                          state={
                            authStatuses.get(selectedPlugin.id) ?? {
                              kind: "loading",
                            }
                          }
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">
                              {t("pluginConfigTab.providedToolsLabel")}
                            </span>
                        ...
                        {toolsExpanded && selectedPlugin.tools.length > 0 && (
                          ...
                          >
                            {selectedPlugin.tools.map((tool) => {
                              const desc =
                                selectedPlugin.toolDescriptions?.[tool];
                              return (
                                <div
                                  key={tool}
                                  className="flex flex-col gap-0.5 rounded border border-border/(--opacity-medium) bg-muted/(--opacity-light) px-2 py-1.5"
                                >
                  ...
                  {selectedPlugin.configSchema ? (
                    ...
                    <div className="space-y-1.5">
                      <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">
                        {t("pluginConfigTab.configSectionLabel")}
                      </span>
                      <PluginConfigSchemaForm
                        ...
                        onSave={async (values) => {
                          ...
                          try {
                            ...
                            showBanner(
                              "success",
                              t("pluginConfigTab.successSaved"),
                            );
                            notifySaved();
                          } catch (e) {
                            showBanner(
                        ...
                        onSetSecret={async (key, value) => {
                          const result =
                            await window.lvis.pluginConfig.setSecret(
                              selectedPlugin.id,
                              key,
                              value,
                            );
                          if (!result.ok) {
                            showBanner(
                              "error",
                              result.message ??
                                t("pluginConfigTab.errorSaveSecret"),
                            );
                            return;
                          }
                          // Optimistically mark the key as present so the
                          // masked "**** (저장됨)" placeholder appears immediately.
                          setSecretsPresent((prev) => ({
                            ...prev,
                            [key]: true,
                          }));
                          showBanner(
                            "success",
                            t("pluginConfigTab.successSaveSecret", { key }),
                          );
                          notifySaved();
                        }}
                      />
                    ...
                    <>
                      ...
                      <div className="flex items-baseline justify-between">
                        ...
                        <span className="text-[10px] text-muted-foreground">
                          {t("pluginConfigTab.envVarsCount", {
                            count: entries.length,
                          })}
                        </span>
                      </div>
                      <ScrollArea
                        className="w-full rounded-md border bg-background/(--opacity-medium) p-2"
                        style={{ maxHeight: 220 }}
                      >
                          ...
                          {entries.map((entry) => (
                            ...
                            >
                              ...
                              <Input
                                ...
                                value={entry.value}
                                onChange={(e) =>
                                  handleUpdateValue(entry.key, e.target.value)
                                }
                              />
                              <Button
                                variant="outline"
                      ...
                      <div className="flex justify-end">
                        ...
                        >
                          {saving
                            ? t("pluginConfigTab.savingLabel")
                            : t("pluginConfigTab.saveButton")}
                        </Button>
                      </div>
                    </>
                  )}
                ...
                <p className="text-xs text-muted-foreground">
                  {t("pluginConfigTab.selectPluginHint")}
                </p>
              )}
            </div>
          </div>
