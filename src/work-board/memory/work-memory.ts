/**
 * Work-flow memory for the personal work board.
 *
 * Two markdown files under `memories/` in the work-board feature namespace:
 *   - `USER.md`   — who the user is (role, focus). Seeded once, never
 *                   auto-overwritten; the user edits it freely.
 *   - `MEMORY.md` — learned work-flow patterns. Bounded by a HARD LINE CAP
 *                   with NO auto-compaction: once the cap is hit the oldest
 *                   lines are dropped, but nothing is summarised or rewritten
 *                   behind the user's back.
 *
 * All reads/writes go through the injected {@link WorkBoardStorage} seam (the
 * host feature namespace) — no raw `fs`, no fallback paths.
 */
 ...
 * lines are dropped to fit (FIFO) — no compaction/summarisation.
 */
...
/**
 * Narrow slice of the storage seam this module depends on. `mkdir` ensures the
 * `memories/` subdir exists before the first write.
 */
...
/**
 * Read `USER.md`, seeding it on first run. The seeded file is written once;
 * subsequent calls return whatever the user has since edited.
 */
 ...
 * when the total would exceed `MEMORY_LINE_CAP`. No summarisation.
 */
export async function appendMemory(
  ...
  // Split off the header block (up to and including the first blank line after
  // the header) so caps only ever evict learned body lines, never the legend.
