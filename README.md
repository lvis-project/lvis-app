...
The current tag-driven public release profile is deliberately **unsigned** while developer registration and signing identities are unavailable. Tag builds are pinned to the GitHub event SHA, receive no signing secrets, and always pass `--skip-code-sign`; their GitHub Release stays a draft until the operator verifies assets and completes its unsigned disclosure record.
