...
describe("PluginPerfTab", () => {
  ...
  it("renders plugin rows with plugin IDs", async () => {
    await renderTab();
    await waitFor(() => {
      expect(screen.getByText("com.ep.meeting")).toBeTruthy();
      expect(screen.getByText("com.ep.email")).toBeTruthy();
      expect(screen.getByText("com.ep.bad")).toBeTruthy();
    });
  });
  ...
  it("color-codes error rate: green for <1%, amber for 1-5%, red for >5%", async () => {
    await renderTab();
    await waitFor(() => {
      // com.ep.meeting: 0/10 = 0% → green
      // com.ep.email: 2/20 = 10% → red (>5%)
      // com.ep.bad: 6/10 = 60% → red
