    ...
    <article>
      ...
      <Tabs defaultValue="catalog" className="my-6">
        ...
        <TabsContent value="catalog">
          ...
          <p>Admins can search the catalog directly / trigger uploads (<code>web/src/pages/AdminPage.tsx useCatalog + apiRequest</code>).</p>
        </TabsContent>
        ...
        <TabsContent value="approvals">
          ...
          <p>
            Pending queue — <code>GET /api/v1/publishes/pending</code> (<code>api/admin.py</code>) +
      ...
      <p>
        A single <code>AuditLog</code> table (<code>models.py</code>) records every admin action. There is no dedicated UI;
        querying directly requires the DB or exporting via an alembic migration.
      </p>

      <Callout tone="security" title="API Key Security">
        ApiKey never stores the raw key — only the sha256 hash + key_prefix are kept in the DB (<code>models.py</code>).
        Verification uses <code>secrets.compare_digest</code> (<code>security.py</code>). Supports revocation + expiry + rotation grace.
      </Callout>
