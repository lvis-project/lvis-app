  ...
  return (
    <article>
      <PageHero
        ...
        tags={["단일 통로", "정적 manifest 기반", "프로세스 경계 도입 중"]}
      />
      ...
      <FeatureGrid
        ...
        items={[
          ...
          { title: "대화 띄우기", body: <>업무 진행 상황을 채팅 본문에 카드로 띄워 사용자가 다른 일을 막지 않게 합니다. 플러그인이 직접 그리는 오버레이 창은 없고, 호스트의 대화 본문이 유일한 표시 자리입니다.</> },
          { title: "비밀값 조회", body: <>API 키 같은 비밀값은 OS 보안 저장소에서 안전하게 가져옵니다. 디스크에 평문으로 저장되지 않습니다. 이 조회는 프로세스 경계를 넘을 수 있어야 하므로 비동기입니다 — 플러그인이 비밀값 묶음을 미리 받아 들고 있는 형태가 아니라, 필요할 때 한 건씩 호스트에 물어보는 형태입니다.</> },
        ]}
      />

      <h2 id="boundary">플러그인은 어디에서 실행되나</h2>
      <p>
        예전에는 모든 플러그인이 호스트와 같은 프로세스 안에서 실행됐습니다. 지금은 <strong>플러그인을 별도의 자식 프로세스에 가두는 경로</strong> 가 만들어져 있고, 그 경로로 실제로 실행되는 플러그인이 하나씩 늘어나는 중입니다.
        오늘 시점에 이 경로를 타는 것은 <code>work-assistant</code> 한 개입니다 (<code>src/plugins/isolation/out-of-process-plugins.ts</code>).
      </p>
      <ul>
        <li><strong>어느 플러그인이 격리되는지는 플러그인이 정하지 않습니다.</strong> 설정 값도, 환경 변수도, 매니페스트 필드도 이 목록을 읽지 않습니다. 목록은 소스에 고정되어 있고, 바뀌려면 코드 변경이 리뷰를 통과해야 합니다.</li>
        <li><strong>자식이 실패하면 호출도 실패합니다.</strong> 같은 호출을 호스트 프로세스 안에서 다시 시도하는 우회 경로는 없습니다.</li>
        <li><strong>통로의 모양은 그대로입니다.</strong> 저장소 · 설정 · 이벤트 구독 · LLM 호출 · 인증 창 · 비밀값 조회 모두 경계 너머로 전달됩니다. 플러그인 입장에서 달라지는 것은 위 “비밀값 조회” 처럼 응답을 기다려야 하는 호출이 있다는 점입니다.</li>
      </ul>

      <Callout tone="info" title="아직 전부는 아닙니다">
