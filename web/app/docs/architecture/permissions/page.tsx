...
export default function Page() {
  return (
    <article>
      ...
      <ul>
        <li><strong>부모가 답할 수 있는 범위에 상한이 있습니다.</strong> 기본값은 ‘중간’ 이고, 이 항목이 받는 값은 ‘낮음’ 과 ‘중간’ 둘뿐입니다 — <strong>높은 위험도는 이 경로를 통과할 수 없고 사용자에게 옵니다</strong>. 상한은 호스트가 부모에게 묻기 <em>전에</em> 적용합니다 — 부모의 답을 나중에 깎는 방식이 아닙니다.</li>
        <li><strong>부모가 보는 근거는 호스트가 구성합니다.</strong> 하위 에이전트가 자기를 변호하려고 쓴 설명 문장은 이 근거에 들어가는 자리가 없습니다. 예외는 <em>판단 대상인 도구 인자</em> 하나입니다 — 그것이 곧 판단할 호출이라 뺄 수 없고, 대신 “하위 에이전트가 쓴 값” 이라고 표시된 채 (그리고 가려진 형태로) 전달됩니다.</li>
        ...
        <li>이 값들은 설정 → 권한에서 볼 수 있습니다. <strong>좁히는 방향으로만 움직이는 것은 위험도 상한 하나뿐입니다</strong> — 시간 · 횟수 · 대화 인용 개수는 기본값보다 <em>올릴 수도</em> 있고, 다만 각각 코드에 박힌 최대치를 넘지 못하도록 잘립니다.</li>
      </ul>

      <Callout tone="security" title="이 경로는 호스트의 하드 검사 뒤에 붙습니다">
        부모에게 질문이 가는 것은 호스트의 하드 검사가 이미 그 호출을 통과시킨 뒤입니다. 부모가 답했다고 해서 이미 거절된 것이 다시 열리지 않고, 부모의 답은 그 호출 한 번에만 적용됩니다 —
        ‘앞으로 계속 허용’ 으로 기억되지 않습니다. 판단 결과는 누가 답했는지 (<code>answeredBy</code>) 와 함께 감사 기록에 남습니다.
      </Callout>
