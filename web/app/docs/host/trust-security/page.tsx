...
export default function Page() {
  return (
    <article>
      ...
      <FeatureGrid
        ...
        items={[
          ...
          },
          {
            ...
            body: <>배포자가 특정 버전을 “쓰지 말 것” 으로 표시하면, 그 버전은 새로 설치되지 않고, 이미 깔려 있어도 다음 실행에서 로드되지 않습니다. 이 회수 목록은 서명되어 있고, <strong>예전 목록을 다시 들이밀어 회수를 취소하는 것</strong> 은 호스트가 거부합니다 — 발행 시각이 지금까지 받은 것보다 과거인 목록은 버립니다. 반대로, 서명된 목록을 <em>한 번도 받지 못한</em> 상태에서는 이 경로가 아무것도 막지 않습니다.</>,
            tone: "ink",
          },
          {
      ...
      <ul>
        <li><strong>사라지지 않고 이유를 남깁니다.</strong> 설치된 플러그인이 로드에 실패하면 조용히 없어지는 것이 아니라, 사이드바의 플러그인 목록에 <strong>실패 사유가 적힌 항목</strong> 으로 남습니다. 매니페스트가 스키마와 안 맞는 경우, 앱 버전이 모자란 경우, 배포자가 그 버전을 회수한 경우, 설치된 파일이 설치 기록과 달라진 경우가 각각 다른 사유로 구분되고, 다시 설치해서 고칠 수 있는 것과 그렇지 않은 것도 구분해 안내합니다.</li>
