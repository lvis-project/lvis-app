    ...
    <article>
      ...
      <FeatureGrid
        ...
        items={[
          ...
          {
            title: "회수된 버전은 다시 뜨지 않습니다",
            body: <>배포자가 특정 버전을 “쓰지 말 것” 으로 표시하면, 그 버전은 새로 설치되지도 않고 이미 깔려 있어도 다음 실행에서 로드되지 않습니다. 이 회수 목록도 서명되어 있고, <strong>예전 목록을 다시 들이밀어 회수를 취소하는 것</strong> 은 호스트가 거부합니다.</>,
            tone: "ink",
          },
          {
      ...
      <h2 id="plugin-boundary">플러그인이 실패할 때</h2>
      <ul>
        <li><strong>사라지지 않고 이유를 남깁니다.</strong> 설치된 플러그인이 로드에 실패하면 조용히 없어지는 것이 아니라, 설정 화면에 <strong>실패 사유가 적힌 카드</strong> 로 남습니다. 매니페스트가 스키마와 안 맞는 경우, 앱 버전이 모자란 경우, 설치된 파일이 설치 기록과 달라진 경우가 각각 다른 사유로 표시되고, 다시 설치해서 고칠 수 있는 것은 그렇게 안내합니다.</li>
        <li><strong>일부는 별도 프로세스에서 실행됩니다.</strong> 플러그인을 호스트와 분리된 자식 프로세스에 가두는 경로가 있고, 그 위로 플러그인을 하나씩 옮기는 중입니다. 자세한 내용과 <strong>오늘 몇 개가 그 경로를 타는지</strong> 는 <a href="/docs/architecture/host-api#boundary">HostApi 문서</a> 에 있습니다.</li>
      </ul>
