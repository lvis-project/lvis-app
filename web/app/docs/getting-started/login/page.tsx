  ...
  return (
    <article>
      <PageHero
        ...
        description="호스트 앱 자체는 로컬-퍼스트로 작동하지만, 플러그인 카탈로그 / 다운로드 / 서명 검증을 위해 Marketplace 계정 인증이 필요합니다. 인증은 plugin 측 hostApi.openAuthWindow / hostApi.openAuthPartitionViewer 로 처리됩니다."
      />
      ...
      <ul>
        ...
        <li><strong>ms-graph, 사내 포털 플러그인</strong> — 각 플러그인 자체 OAuth (MSAL · 포털 SSO). 토큰은 plugin namespace 에 격리.</li>
      </ul>
      ...
      <StepList
        steps={[
          ...
          { title: "Plugin OAuth — 필요 시", body: <p>ms-graph (MSAL) / 사내 포털 플러그인 (포털 SSO) 은 plugin install 후 첫 사용 시 <code>hostApi.openAuthWindow</code> 로 별도 처리.</p> },
        ]}
      />
      ...
      <Callout tone="tip" title="로그인 없이도 쓸 수 있는 범위">
        ...
        다만 플러그인 신규 설치와 Marketplace 카탈로그는 비활성화됩니다.
      </Callout>
